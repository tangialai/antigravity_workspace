# ODOO DOCUMENTATION SUMMARY - CATEGORY: ODOO19DOCS_CONTRIBUTING
...
:   You should **NEVER** call `cr.commit()` or `cr.rollback()` yourself,
    **UNLESS** you have explicitly created your own database cursor! And
    the situations in which you need to do this are exceptional!
...
::: seealso
`reference/actions/cron`{.interpreted-text role="ref"}
:::
...
savepoints have a huge overhead. If you process records and savepoint in
a loop, for example when processing records one by one for a batch,
