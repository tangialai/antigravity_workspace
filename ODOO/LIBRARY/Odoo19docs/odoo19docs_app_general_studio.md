# ODOO DOCUMENTATION SUMMARY - CATEGORY: ODOO19DOCS_APP_GENERAL_STUDIO
...
- `fa-users`{.interpreted-text role="icon"} `(users)`{.interpreted-text
...
To schedule a new activity, click the `Activity`{.interpreted-text
...
![Activity button in chatter field.](activities/chatter.png)
...
![Top-right menu with the Activities icon called out.](activities/activities.png)
...
Colored bars in each column represent records for specific activity
types, and display a number indicating how many activities are scheduled
for that type.
...
`(plus)`{.interpreted-text role="guilabel"} icon, and then
`fill out the Schedule Activity form
<activities/form>`{.interpreted-text role="ref"}.

![Activity view of the CRM pipeline and the option to schedule an activity.](activities/activity-view.png)
...
- `fa-check`{.interpreted-text role="icon"} `To-Do`{.interpreted-text
  role="guilabel"}
- `fa-envelope`{.interpreted-text role="icon"} `Email`{.interpreted-text
  role="guilabel"}
- `fa-phone`{.interpreted-text role="icon"} `Call`{.interpreted-text
  role="guilabel"}
- `fa-users`{.interpreted-text role="icon"} `Meeting`{.interpreted-text
  role="guilabel"}
- `Do Stuff`{.interpreted-text role="guilabel"}
- `fa-upload`{.interpreted-text role="icon"}
  `Document`{.interpreted-text role="guilabel"}
- `fa-pencil-square-o`{.interpreted-text role="icon"}
  `Signature`{.interpreted-text role="guilabel"}
...
- `Log a note`{.interpreted-text role="guilabel"}: add any additional
...
Click `Save`{.interpreted-text role="guilabel"} to schedule the
activity, and add the activity to the chatter under
`Planned Activities`{.interpreted-text role="guilabel"}. Click
`Mark Done`{.interpreted-text role="guilabel"} to add the details of the
activity to the chatter under `Today`{.interpreted-text
role="guilabel"}.

#### Schedule a meeting

If `fa-users`{.interpreted-text role="icon"} `Meeting`{.interpreted-text
role="guilabel"} is selected as the activity type, enter the information
as instructed above, then click `Schedule`{.interpreted-text
role="guilabel"}. This opens the user\'s calendar to add and schedule
the activity.
...
If any activities are scheduled, the number of activities appears in a
...
![The list of activities that is accessed from the main menu bar.](activities/activities-menu.png)
:::
...
![Activity Types button in the Settings application under the Discuss section.](activities/settings-activities-types.png)
...
![The list of activity types already configured and available.](activities/activity-list.png)
...
activity type form loads. Enter a `Name`{.interpreted-text
role="guilabel"} for the activity type at the top of the form, then
...
![A new Activity form with all the fields filled out.](activities/new-activity.png)
...
company does business with through Odoo. A contact record is a
repository of vital business information, facilitating communication and
...
Odoo supports both `Person`{.interpreted-text role="guilabel"} and
`Company`{.interpreted-text role="guilabel"} contacts. Select the
appropriate type for the new contact. Fill in the name of the
`Person`{.interpreted-text role="guilabel"} or
`Company`{.interpreted-text role="guilabel"}. This is how the name
...
`Person`{.interpreted-text role="guilabel"} contacts can be linked to a
`Company`{.interpreted-text role="guilabel"} contact. After selecting
`Person`{.interpreted-text role="guilabel"}, the
`Company`{.interpreted-text role="guilabel"} field becomes visible.
::::

In the `fa-envelope`{.interpreted-text role="icon"}
`(envelope)`{.interpreted-text role="guilabel"} field, enter the email
address with the domain. In the `fa-phone`{.interpreted-text
role="icon"} `(phone)`{.interpreted-text role="guilabel"} field enter
the phone number with the appropriate country code. Next, enter the
`Address`{.interpreted-text role="guilabel"} of the
`Company`{.interpreted-text role="guilabel"} or
`Person`{.interpreted-text role="guilabel"}.
...
- `Job Position`{.interpreted-text role="guilabel"}: list the job
  position of the `Person`{.interpreted-text role="guilabel"} (only
  available on `Person`{.interpreted-text role="guilabel"}).
...
On the `Contacts`{.interpreted-text role="guilabel"} tab, contacts can
be added that are associated with a `Company`{.interpreted-text
role="guilabel"} and related addresses. For example, a specific contact
person for the company can be listed here.

Multiple addresses can be added on both `Person`{.interpreted-text
role="guilabel"} and `Company`{.interpreted-text role="guilabel"}
contacts. To do so, click `Add Contact`{.interpreted-text
role="guilabel"} in the `Contacts`{.interpreted-text role="guilabel"}
tab. Doing so reveals a `Create Contact`{.interpreted-text
role="guilabel"} pop-up form, in which additional addresses can be
configured.
...
- `Invoice`{.interpreted-text role="guilabel"}: adds a specific invoice
  address to the existing contact form.
- `Delivery`{.interpreted-text role="guilabel"}: adds a specific
  delivery address to the existing contact form.
- `Other`{.interpreted-text role="guilabel"}: adds an alternate address
  to the existing contact form.

![Create a new contact/address on a contact form.](contacts/create-contact-window.png)
...
been selected. This is similar to the `Person`{.interpreted-text
...
Under the `Purchase`{.interpreted-text role="guilabel"} heading, select
how `RFQs (requests for quotation)`{.interpreted-text role="abbr"} for
this contact should be grouped together:

- `On Order`{.interpreted-text role="guilabel"}: replenishment needs are
  grouped together except for `MTO (made to
  order)`{.interpreted-text role="abbr"}
- `Daily`{.interpreted-text role="guilabel"}: replenishment needs are
  grouped if the expected arrival is the same day.
- `Weekly`{.interpreted-text role="guilabel"}: replenishment needs are
  grouped if the expected arrival is the same week or week day.
- `Always`{.interpreted-text role="guilabel"}: replenishment needs are
  always grouped.

Select a default `Buyer`{.interpreted-text role="guilabel"} if the
`RFQs (requests for quotation)`{.interpreted-text role="abbr"} should
always be assigned to the same user.
...
as well. Select a `Supplier Currency`{.interpreted-text role="guilabel"}
to be used for purchases from the contact.

#### Fiscal information

To assign a `Fiscal Position`{.interpreted-text role="guilabel"} to this
contact, select it from the drop-down menu.

#### Misc section

Under the `Misc.`{.interpreted-text role="guilabel"} heading, enter a
`Citizen Identification`{.interpreted-text role="guilabel"} number for
*Person* contacts, or `Company ID`{.interpreted-text role="guilabel"}
for *Company* contacts. Use `Reference`{.interpreted-text
...
Select one or more `Website Tags`{.interpreted-text role="guilabel"} to
assist in filtering published customers on the [/customers]{.title-ref}
website page. Select an `Industry`{.interpreted-text role="guilabel"}
for this contact from the drop-down menu. Use the
`SLA Policies`{.interpreted-text role="guilabel"} field to assign a
*Helpdesk* SLA policy to this contact.

#### Inventory section

Under the `Inventory`{.interpreted-text role="guilabel"} heading, use
the `Customer Location`{.interpreted-text role="guilabel"} drop-down to
select the stock location to be used as a destination when sending goods
to this contact. Next, use the `Vendor Location`{.interpreted-text
role="guilabel"} drop-down to selection the stock location used as a
source when recieving goods from this contact. Finally, use the
`Subcontractor Location`{.interpreted-text role="guilabel"} drop-down to
idenfy a location used as both a source and destination when sending
...
Under the
`Invoice Follow-Ups <accounting/follow_up/follow-ups-for-one-customer>`{.interpreted-text
role="ref"} heading, the `Follow-up Status`{.interpreted-text
role="guilabel"} indicates if the contact currently has any overdue
payments. To configure `Reminders`{.interpreted-text role="guilabel"}
for this contact, use the radio buttons to select either
`Automatic`{.interpreted-text role="guilabel"} or
`Manual`{.interpreted-text role="guilabel"} reminders, as well as
schedule the `Next
Reminder`{.interpreted-text role="guilabel"}.
...
*Holdings* information from the `Equity`{.interpreted-text
role="guilabel"} app, as well as information for the individual contact,
including their `Birth Date`{.interpreted-text role="guilabel"} and
`Birth Place`{.interpreted-text role="guilabel"}.

The `UBO`{.interpreted-text role="guilabel"} tab is **only** present
when the *Equity* app is installed.

To request the information required for the
`UBO (Ultimate Beneficial Owner)`{.interpreted-text role="abbr"} tab,
click the `fa-cog`{.interpreted-text role="icon"}
`Action`{.interpreted-text role="guilabel"} icon, then select
`Request UBO Form`{.interpreted-text role="guilabel"} to send an email
to the contact.
...
The Odoo rich-text editor allows for creating and editing rich-text
content in HTML fields, such as the `Internal Notes`{.interpreted-text
...
role="ref"} and the `Studio report editor
<studio/pdf-reports/report-editor>`{.interpreted-text role="ref"}, among
others. Start typing or use the `toolbar
<essentials/html_editor/toolbar>`{.interpreted-text role="ref"} or
`powerbox <essentials/html_editor/commands>`{.interpreted-text
role="ref"} for formatting and structuring text.
...
display the text editor toolbar and apply any of the following standard
formatting options:

- **Font style**: Define the font style using various options, such as
  `Header 1 to 6,
...
- `fa-bold`{.interpreted-text role="icon"}
  (`Toggle bold`{.interpreted-text role="guilabel"}): Put the text in
  bold.
- `fa-italic`{.interpreted-text role="icon"}
  (`Toggle italic`{.interpreted-text role="guilabel"}): Put the text in
  italics.
- `fa-underline`{.interpreted-text role="icon"}
  (`Toggle underline`{.interpreted-text role="guilabel"}): Underline the
  text.
- `fa-font`{.interpreted-text role="icon"}
  (`Apply Font Color`{.interpreted-text role="guilabel"}): Customize the
  font colors:
  ...
  - `fa-trash`{.interpreted-text role="icon"} (`Reset`{.interpreted-text
    role="guilabel"}): Restore the original font/background color.
- `fa-link`{.interpreted-text role="icon"}
  (`Add a link`{.interpreted-text role="guilabel"}): Insert or edit a
  URL link to a selected text, and optionally upload an image using its
  file URL.
- `Odoo AI`{.interpreted-text role="guilabel"}: Write a prompt and get
  AI-generated content. Optionally, click the AI suggestions instead of
  writing a prompt.

Click the `fa-ellipsis-v`{.interpreted-text role="icon"}
(`Expand toolbar`{.interpreted-text role="guilabel"}) icon to display
additional formatting options:

- **Font family**: Use the `Default system font`{.interpreted-text
  role="guilabel"} or select a preferred font family for the text.
- `fa-strikethrough`{.interpreted-text role="icon"}
  (`Toggle strikethrough`{.interpreted-text role="guilabel"}): Strike
  through the text.
- `fa-paint-brush`{.interpreted-text role="icon"}
  `(Apply Background Color)`{.interpreted-text role="guilabel"}:
  Customize the background colors.
- `fa-eraser`{.interpreted-text role="icon"}
  (`Remove Format`{.interpreted-text role="guilabel"}): Remove all
  formatting applied to a selected text.
- `fa-list-ul`{.interpreted-text role="icon"}
  (`Toggle List`{.interpreted-text role="guilabel"}): Select the
  ...
  - `fa-check-square-o`{.interpreted-text role="icon"}
    (`Checklist`{.interpreted-text role="guilabel"}): Turn the text into
    a checklist.
- `fa-align-left`{.interpreted-text role="icon"}
  (`Align text`{.interpreted-text role="guilabel"}): Select the
  following text alignments:
  - `fa-align-left`{.interpreted-text role="icon"}
    (`align left`{.interpreted-text role="guilabel"}): Align the text on
    the left.
  - `fa-align-center`{.interpreted-text role="icon"}
    (`align center`{.interpreted-text role="guilabel"}): Align the text
    in the center.
  - `fa-align-right`{.interpreted-text role="icon"}
    (`align right`{.interpreted-text role="guilabel"}): Align the text
    on the right.
  - `fa-align-justify`{.interpreted-text role="icon"}
    (`justify`{.interpreted-text role="guilabel"}): Apply straight edges
    to both text margins.
- `Translate with AI`{.interpreted-text role="guilabel"}: Translate the
  content in the `installed languages
...
- **Emphasis**: Press [CTRL]{.title-ref}/\`CMD\` + [B]{.title-ref},
  ...
  [CTRL]{.title-ref}/\`CMD\` + [U]{.title-ref} to apply the bold,
  italics, or underlined effect.
- **Numbered list**: Type [1.]{.title-ref}, [1)]{.title-ref},
  [A.]{.title-ref}, or [A)]{.title-ref} to start a numbered list.
- **Bulleted list**: Type [\*]{.title-ref} or [-]{.title-ref} to start a
  bulleted list.

\- Click a hyperlinked text and perform one of the following actions:
`fa-clipboard`{.interpreted-text role="icon"}
(`Copy Link`{.interpreted-text role="guilabel"}),
`fa-pencil-square-o`{.interpreted-text role="icon"}
(`Edit Link`{.interpreted-text role="guilabel"}), or
`fa-chain-broken`{.interpreted-text role="icon"}
(`Remove Link`{.interpreted-text role="guilabel"}).
::::

## Powerbox commands {#essentials/html_editor/commands}

Commands enable editing and managing various types of features within
the text editor, such as tables, banners, headers, and more.
...
.. tab:: Structure
  ...
  Command                                       Use
  --------------------------------------------- --------------------------------------------------------
  `fa-minus`{.interpreted-text role="icon"}     Insert a horizontal rule separator.
  `Separator`{.interpreted-text                 
  role="guilabel"}                              

  `fa-columns`{.interpreted-text role="icon"}   Convert into 2 columns.
  `2 columns`{.interpreted-text                 
  role="guilabel"}                              

  `fa-columns`{.interpreted-text role="icon"}   Convert into 3 columns.
  `3 columns`{.interpreted-text                 
  role="guilabel"}                              

  `fa-columns`{.interpreted-text role="icon"}   Convert into 4 columns.
  `4 columns`{.interpreted-text                 
  role="guilabel"}                              

  `fa-table`{.interpreted-text role="icon"}     Insert a table.
  `Table`{.interpreted-text role="guilabel"}    

  `fa-list-ul`{.interpreted-text role="icon"}   Create a bulleted list.
  `Bulleted list`{.interpreted-text             
  role="guilabel"}                              

  `fa-list-ol`{.interpreted-text role="icon"}   Create a numbered list.
  `Numbered list`{.interpreted-text             
  role="guilabel"}                              

  `fa-check-square-o`{.interpreted-text         Create a checklist.
  role="icon"} `Checklist`{.interpreted-text    
  role="guilabel"}                              

  `fa-caret-square-o-right`{.interpreted-text   Hide a group of text under a foldable toggle.
  role="icon"} `Toggle list`{.interpreted-text  
  role="guilabel"}                              
  ------------------------------------------------------------------------------------------------------
...
:::
  ...
  Command                                       Use
  --------------------------------------------- -------------------------------------------------
  `fa-info-circle`{.interpreted-text            Insert an info banner.
  role="icon"} `Banner Info`{.interpreted-text  
  role="guilabel"}                              

  `fa-check-circle`{.interpreted-text           Insert a success banner.
  role="icon"}                                  
  `Banner Success`{.interpreted-text            
  role="guilabel"}                              

  `fa-exclamation-triangle`{.interpreted-text   Insert a warning banner.
  role="icon"}                                  
  `Banner Warning`{.interpreted-text            
  role="guilabel"}                              

  `fa-exclamation-circle`{.interpreted-text     Insert a danger banner.
  role="icon"}                                  
  `Banner Danger`{.interpreted-text             
  role="guilabel"}                              
  -----------------------------------------------------------------------------------------------
...
:::
  ...
  Command                                Use
  -------------------------------------- -------------------------------------------------
  `fa-header`{.interpreted-text          Big section heading.
  role="icon"}                           
  `Heading 1`{.interpreted-text          
  role="guilabel"}                       

  `fa-header`{.interpreted-text          Medium section heading.
  role="icon"}                           
  `Heading 2`{.interpreted-text          
  role="guilabel"}                       

  `fa-header`{.interpreted-text          Small section heading.
  role="icon"}                           
  `Heading 3`{.interpreted-text          
  role="guilabel"}                       

  `fa-paragraph`{.interpreted-text       Paragraph block: Insert a paragraph.
  role="icon"} `Text`{.interpreted-text  
  role="guilabel"}                       

  `fa-exchange`{.interpreted-text        Switch the text\'s direction.
  role="icon"}                           
  `Switch direction`{.interpreted-text   
  role="guilabel"}                       

  `fa-quote-right`{.interpreted-text     Add a blockquote section.
  role="icon"} `Quote`{.interpreted-text 
  role="guilabel"}                       

  `fa-code`{.interpreted-text            Add a code section.
  role="icon"} `Code`{.interpreted-text  
  role="guilabel"}                       
  ----------------------------------------------------------------------------------------
...
:::
  ...
  Command                               Use
  ------------------------------------- ------------------------------------------------------------------
  `fa-file-image-o`{.interpreted-text   `Insert an image <insert-media>`{.interpreted-text role="ref"} or
  role="icon"}                          icon: `Search the Unsplash database
  `Media`{.interpreted-text             </applications/general/integrations/unsplash>`{.interpreted-text
  role="guilabel"}                      role="doc"} or upload images, documents, or icons.

  `fa-upload`{.interpreted-text         Add a download box: share images, recordings, or documents that
  role="icon"}                          internal users can download.
  `Upload a file`{.interpreted-text     
  role="guilabel"}                      
  --------------------------------------------------------------------------------------------------------
...
:::
  ...
  Command                                 Use
  --------------------------------------- -------------------------------------------------
  `fa-link`{.interpreted-text             Add a link: Type the label and enter a URL or
  role="icon"} `Link`{.interpreted-text   upload a file, then click
  role="guilabel"}                        `Apply`{.interpreted-text role="guilabel"}.

  `fa-link`{.interpreted-text             Add a button: Type the label, enter a URL or
  role="icon"} `Button`{.interpreted-text upload a file, select the button style, type, and
  role="guilabel"}                        size, then click `Apply`{.interpreted-text
                                          role="guilabel"}.

  `fa-bookmark`{.interpreted-text         Highlight the structure (headings): Create a
  role="icon"}                            table of content based on the headings.
  `Table Of Contents`{.interpreted-text   
  role="guilabel"}                        
  -----------------------------------------------------------------------------------------
...
:::
  ...
  Command                          Use
  -------------------------------- --------------------------------------------------------
  `fa-smile-o`{.interpreted-text   Add an emoji: search for the desired emoji.
  role="icon"}                     
  `Emoji`{.interpreted-text        
  role="guilabel"}                 

  `fa-star-o`{.interpreted-text    Insert a rating of up to 3 stars.
  role="icon"}                     
  `3 Stars`{.interpreted-text      
  role="guilabel"}                 

  `fa-star`{.interpreted-text      Insert a rating of up to 5 stars.
  role="icon"}                     
  `5 Stars`{.interpreted-text      
  role="guilabel"}                 
  -----------------------------------------------------------------------------------------
::::::::::
...
`(media)`{.interpreted-text role="guilabel"} icon in the tooltip, then
...
- `Icons`{.interpreted-text role="guilabel"}: Search for and select one
  of the available icons.

- `Videos`{.interpreted-text role="guilabel"}: Paste a video URL of the
  following sources: YouTube, Vimeo, Dailymotion, and Youku.
  Alternatively, type code to embed a video.
...
- `fa-search-plus`{.interpreted-text role="icon"}
  (`Preview image`{.interpreted-text role="guilabel"}): Preview the
  image, zoom in or out, rotate it, print it, or download it. Exit the
  preview by clicking the `fa-times`{.interpreted-text role="icon"}
  `(close)`{.interpreted-text role="guilabel"} icon in the top right
  corner.

- `Description`{.interpreted-text role="guilabel"}: Edit the image
  description and tooltip, then click `Apply`{.interpreted-text
  role="guilabel"}.

- `Caption`{.interpreted-text role="guilabel"}: Write a caption under
  100 characters below the image.
...
- `fa-picture-o`{.interpreted-text role="icon"}
  (`Thumbnail`{.interpreted-text role="guilabel"}): Apply a border to
  the image.

- `fa-plus-square-o`{.interpreted-text role="icon"}
  (`Padding`{.interpreted-text role="guilabel"}): Add an image padding
  and choose from `Small`{.interpreted-text role="guilabel"},
  `Medium`{.interpreted-text role="guilabel"}, `Large`{.interpreted-text
  role="guilabel"}, or `XL`{.interpreted-text role="guilabel"} sizes.

- `Resize`{.interpreted-text role="guilabel"}: Restore the image to its
  default size or set its size to `25%`{.interpreted-text
  role="guilabel"}, `50%`{.interpreted-text role="guilabel"}, or
...
- `fa-crop`{.interpreted-text role="icon"}
  (`Crop image`{.interpreted-text role="guilabel"}): Crop the image
  manually or apply the following options:
...
- `fa-link`{.interpreted-text role="icon"}
  (`Add a link`{.interpreted-text role="guilabel"}): Add a link to the
  image, type the URL, then click `Apply`{.interpreted-text
  role="guilabel"}. To remove the link, click
  `fa-unlink`{.interpreted-text role="icon"}
  (`Remove Link`{.interpreted-text role="guilabel"}).
...
- `fa-trash`{.interpreted-text role="icon"} (`Delete`{.interpreted-text
  role="guilabel"}): Delete the image.

#### Icon editor toolbar

After `inserting an icon <insert-media>`{.interpreted-text role="ref"},
click it to display the icon editor toolbar, and apply any of the
following formatting options:

- `fa-font`{.interpreted-text role="icon"}
  (`Select Font Color`{.interpreted-text role="guilabel"}): Customize
  the font color.
- `fa-paint-brush`{.interpreted-text role="icon"}
  (`Select Background Color`{.interpreted-text role="guilabel"}):
  Customize the background color.
- `Resize icon`{.interpreted-text role="guilabel"}: From
  `1x`{.interpreted-text role="guilabel"} to `5x`{.interpreted-text
  role="guilabel"}.
- `fa-play`{.interpreted-text role="icon"}
  (`Toggle icon spin`{.interpreted-text role="guilabel"}): Activate the
  spin animation.
- `Replace`{.interpreted-text role="guilabel"}: Select a different icon.
...
`Edit Properties`{.interpreted-text role="guilabel"}.
...
+---------------------------------------------------------------------------------+---------------------------------------------------------------------+----------------------------------------------------+
| `Multiline Text <studio/fields/simple-fields-multiline-text>`{.interpreted-text | Full text on multiple lines                                         | Enter a `Default Value`{.interpreted-text          |
| role="ref"}                                                                     |                                                                     | role="guilabel"} if desired.                       |
+---------------------------------------------------------------------------------+---------------------------------------------------------------------+----------------------------------------------------+
| `HTML <studio/fields/simple-fields-html>`{.interpreted-text role="ref"}         | HTML field                                                          | Enter a `Default Value`{.interpreted-text          |
|                                                                                 |                                                                     | role="guilabel"} if desired.                       |
+---------------------------------------------------------------------------------+---------------------------------------------------------------------+----------------------------------------------------+
| `Checkbox <studio/fields/simple-fields-checkbox>`{.interpreted-text role="ref"} | Checked or unchecked status                                         | Choose the `Default State`{.interpreted-text       |
|                                                                                 |                                                                     | role="guilabel"}.                                  |
+---------------------------------------------------------------------------------+---------------------------------------------------------------------+----------------------------------------------------+
| `Integer <studio/fields/simple-fields-integer>`{.interpreted-text role="ref"}   | Integer numbers                                                     | Enter a `Default Value`{.interpreted-text          |
|                                                                                 | (`positive, negative, or zero, without a decimal`{.interpreted-text | role="guilabel"} if desired.                       |
|                                                                                 | role="dfn"})                                                        |                                                    |
+---------------------------------------------------------------------------------+---------------------------------------------------------------------+----------------------------------------------------+
| `Decimal <studio/fields/simple-fields-decimal>`{.interpreted-text role="ref"}   | Decimal numbers                                                     | Enter a `Default Value`{.interpreted-text          |
|                                                                                 | (`positive, negative, or zero, with a decimal`{.interpreted-text    | role="guilabel"} if desired.                       |
|                                                                                 | role="dfn"})                                                        |                                                    |
+---------------------------------------------------------------------------------+---------------------------------------------------------------------+----------------------------------------------------+
| `Monetary <studio/fields/simple-fields-monetary>`{.interpreted-text role="ref"} | Selection of a (cost) currency                                      | Enter a `Default Value`{.interpreted-text          |
|                                                                                 |                                                                     | role="guilabel"} if desired.                       |
+---------------------------------------------------------------------------------+---------------------------------------------------------------------+----------------------------------------------------+
...
|                                                                                 |                                                                     | role="guilabel"}) button.                          |
+---------------------------------------------------------------------------------+---------------------------------------------------------------------+----------------------------------------------------+
| `Tags <studio/fields/relational-fields-tags>`{.interpreted-text role="ref"}     | Selection of multiple values in the form of tags                    | Enter a `Tag`{.interpreted-text role="guilabel"}   |
|                                                                                 |                                                                     | name and press [Enter]{.title-ref} to save it.     |
|                                                                                 |                                                                     |                                                    |
...
\- Enter a `Suffix`{.interpreted-text role="guilabel"} to add a
contextual description after a field\'s value. For example, to indicate
the cost per kilometer, use the `Monetary`{.interpreted-text
role="guilabel"} field and name it **Cost**, then enter **per km** as
the `Suffix`{.interpreted-text role="guilabel"} field. The property then
displays the following: **Cost** \[added value\] **per km**. - Enable
`Display in Cards`{.interpreted-text role="guilabel"} to select whether
to display the property in the Kanban, List, or Calendar views\' cards
for every field. - Enable `AI`{.interpreted-text role="guilabel"} to add
`AI fields <ai/add-a-new-field/properties>`{.interpreted-text
role="ref"} to the property. Write a `Prompt`{.interpreted-text
role="guilabel"} and/or type [/field]{.title-ref} to insert dynamic
values. - To add another property, click `fa-plus`{.interpreted-text
role="icon"} `Add a Property`{.interpreted-text role="guilabel"} at the
bottom of the form.
::::
...
- Click `fa-trash`{.interpreted-text role="icon"}
  ...
  `Delete`{.interpreted-text role="guilabel"} to permanently remove it.
...
> | role="guilabel"}                  |                                                                                                        |                                                                                             |
> +-----------------------------------+--------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------+
> | `Approvals`{.interpreted-text     | Approval Request                                                                                       | Category                                                                                    |
> | role="guilabel"}                  |                                                                                                        |                                                                                             |
> +-----------------------------------+--------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------+
...
> | role="guilabel"}                  |                                                                                                        |                                                                                             |
> +-----------------------------------+--------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------+
> | `Payroll`{.interpreted-text       | `Employee <employees/general-info>`{.interpreted-text role="ref"}                                      | `Company <employees/general-info>`{.interpreted-text role="ref"}                            |
> | role="guilabel"}                  |                                                                                                        |                                                                                             |
> |                                   | `Payslip <payroll/new-payslip>`{.interpreted-text role="ref"}                                          | `Structure <payroll/structure-types>`{.interpreted-text role="ref"}                         |
> +-----------------------------------+--------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------+
...
The following models do not depend on any *parent* and apply to all
records:

- `Contact Form <../../../applications/essentials/contacts>`{.interpreted-text
  role="doc"} in the `Contacts`{.interpreted-text role="guilabel"} app
- `Mailing List Contacts <marketing/mailing-list/contacts>`{.interpreted-text
  role="ref"} in the `Email
  Marketing`{.interpreted-text role="guilabel"} app
::::
...
while any `shared
favorites <search/favorites-share>`{.interpreted-text role="ref"},
whether created by the current user or another user, are shown in a
second section.

To edit, archive, or remove a favorited search, hover over the search
name in the list of favorites and click `fa-pencil`{.interpreted-text
role="icon"} `(Edit favorite)`{.interpreted-text role="guilabel"}. The
following fields can be modified: `Filter Name`{.interpreted-text
role="guilabel"},
`Shared with <search/favorites-share>`{.interpreted-text role="ref"},
`Default Filter`{.interpreted-text role="guilabel"},
`Domain`{.interpreted-text role="guilabel"}. To archive or remove the
favorite, click `fa-cog`{.interpreted-text role="icon"}
`(Actions)`{.interpreted-text role="guilabel"} then
`oi-archive`{.interpreted-text role="icon"} `Archive`{.interpreted-text
role="guilabel"} or `fa-trash`{.interpreted-text role="icon"}
`Delete`{.interpreted-text role="guilabel"}, as appropriate.
...
\- When a favorited search is in use, additional filters or groups can
be used to further refine the search. The conditions applied by the
favorited search can also be modified by hovering over the
`fa-star`{.interpreted-text role="icon"} `(star)`{.interpreted-text
role="guilabel"} beside the search name then clicking the
`fa-cog`{.interpreted-text role="icon"} `(cog)`{.interpreted-text
role="guilabel"} icon. These changes only affect the current search. -
To edit the grouping or sorting of a favorited search,
`activate developer mode
<developer-mode>`{.interpreted-text role="ref"} when editing the search,
then modify the `Context`{.interpreted-text role="guilabel"} or
`Sort`{.interpreted-text role="guilabel"} fields as needed. - To view
*all* favorited searches, with developer mode activated, navigate to
...
### Share a favorited search {#search/favorites-share}

By default, a favorited search is only available to the user who created
it. To share a favorited search with other users, click
`Edit`{.interpreted-text role="guilabel"} when creating the favorite,
or, for an existing favorite, click `fa-pencil`{.interpreted-text
role="icon"} `(Edit favorite)`{.interpreted-text role="guilabel"} after
hovering over the search name in the list of favorites.

In the `Shared with`{.interpreted-text role="guilabel"} field, select
the relevant users from the drop-down menu, then click
`fa-cloud-upload`{.interpreted-text role="icon"}
`(Save manually)`{.interpreted-text role="guilabel"}. The favorite is
now visible for all selected users under `fa-star`{.interpreted-text
role="icon"} `Favorites`{.interpreted-text role="guilabel"}.
...
The *Days to rot* feature provides proactive notification when a
specific card within that stage requires attention. When a card passes
the defined rotting threshold, it becomes highlighted in red and the
stage displays the total number of cards currently rotting.

![Sample of a rotting card and stage in CRM app.](stages/rotting-card.png)

To enable this feature, the Kanban view must be active. Navigate to the
desired app and click on the `oi-view-kanban`{.interpreted-text
role="icon"} `(Kanban)`{.interpreted-text role="guilabel"} icon.

To set the threshold for a stage, click on the
`fa-gear`{.interpreted-text role="icon"} `(Settings)`{.interpreted-text
role="guilabel"} icon for the relevant stage and select
`Edit`{.interpreted-text role="guilabel"} from the drop-down menu. A
configuration window will appear.

In the `Days to rot`{.interpreted-text role="guilabel"} field, enter the
number of days that marks the threshold before action is needed. Click
`Save`{.interpreted-text role="guilabel"} to apply the configuration.

![Sample of a configuration window with a rotting threshold in the CRM app.](stages/days-to-rot-field.png)
...
![Digest Emails section inside General Settings.](digest_emails/digest-email-settings-screenshot.png)
...
`Settings --> Emails section`{.interpreted-text role="menuselection"}.
Then, deactivate the `Digest Emails`{.interpreted-text role="guilabel"}
feature, by un-ticking the checkbox, and clicking
`Save`{.interpreted-text role="guilabel"}. See the section on
...
to `Settings
app --> Emails section --> Digest Email field`{.interpreted-text
role="menuselection"}. Then, select
`Your Odoo Periodic Digest`{.interpreted-text role="guilabel"}, and
click on the `↗️ (External link)`{.interpreted-text role="guilabel"}
icon, next to the drop-down menu selection.
...
First, navigate to `Settings app --> Emails section`{.interpreted-text
role="menuselection"}, and click `Configure
Digest Emails`{.interpreted-text role="guilabel"}.
...
To remove a user as a recipient, click the `fa-times`{.interpreted-text
role="icon"} `(cancel)`{.interpreted-text role="guilabel"} icon to the
far-right of the user listed in the `Recipients`{.interpreted-text
role="guilabel"} tab.
...
role="menuselection"}, and click
`Configure Digest Emails`{.interpreted-text role="guilabel"}. Then,
click `New`{.interpreted-text role="guilabel"} to create a new digest
email.
...
## Branches[]{#general/companies/branches} {#general/branches}
...
::: seealso
Learn how Odoo handles your data by reading Odoo\'s [Privacy
Policy](https://www.odoo.com/privacy) and
`Terms and Conditions <../../../../legal>`{.interpreted-text
role="doc"}.
:::
...
First, enable the *Mail Plugin* module in the database. Go to
`Settings --> General
...
this, click on the `Apps`{.interpreted-text role="guilabel"} button in
the upper right-side, then click `Get add-ins`{.interpreted-text
role="guilabel"}.

![Apps button in Outlook](outlook/more-actions.png)
...
Under `Custom Addins`{.interpreted-text role="guilabel"} towards the
bottom, click on `+ Add a custom add-in`{.interpreted-text
role="guilabel"}, then select `Add from file...`{.interpreted-text
role="guilabel"}
...
email in the Outlook mailbox, click on the `Apps`{.interpreted-text
role="guilabel"} button in the upper right-side, and select `Odoo Inbox
Addin`{.interpreted-text role="guilabel"}.
...
## Using the inbox plugin {#mail-plugin/outlook/add-shortcut}

To use the plugin, open any email in the Outlook mailbox, click on the
`Apps`{.interpreted-text role="guilabel"} button in the upper
right-side, and select `Odoo Inbox Addin`{.interpreted-text
role="guilabel"} to open the panel.
...
To create a contact record in Odoo from an email, click the
`fa-plus-circle`{.interpreted-text role="icon"}
`(circle plus)`{.interpreted-text role="guilabel"} icon.
...
To create an opportunity from an email, click the
`fa-plus`{.interpreted-text role="icon"} `(plus)`{.interpreted-text
role="guilabel"} icon next to `Opportunities`{.interpreted-text
role="guilabel"}. Doing so opens a new opportunity record in Odoo in a
new tab. The subject of the email is used as the opportunity title, and
the content of the email is added to the `Notes`{.interpreted-text
role="guilabel"} tab of the opportunity.

### Create a task

To create a task from an email, click the `fa-plus`{.interpreted-text
role="icon"} `(plus)`{.interpreted-text role="guilabel"} icon next to
`Tasks`{.interpreted-text role="guilabel"}. Then, enter the title of the
project where the task should be created in the resulting field, and
select the appropriate project from the resulting list. This opens a new
task record in Odoo in a new tab. The subject of the email is used as
the task title, and the content of the email is added to the
...
To create a task from an email, click the `fa-plus`{.interpreted-text
role="icon"} `(plus)`{.interpreted-text role="guilabel"} icon next to
`Tickets`{.interpreted-text role="guilabel"}. Doing so opens a new
ticket record in Odoo in a new tab. The subject of the email is used as
the ticket title, and the content of the email is added to the
`Description`{.interpreted-text role="guilabel"} tab of the ticket.
...
- The device connecting to Odoo must be on the same network as the IoT
  system.
...
The browser then scans the network to which the computer is connected
for available IoT systems. If an IoT system is detected, it is connected
automatically. If multiple IoT systems are found, select the appropriate
one in the popup that opens, then click `Connect`{.interpreted-text
role="guilabel"}. If no IoT system is detected automatically, try
connecting it to the database using a `pairing code
...
The IoT system's name is composed of either the
`IoT box <iot_box>`{.interpreted-text role="doc"} or the
`Windows computer <windows_iot>`{.interpreted-text role="doc"}'s
motherboard serial number, followed by its `pairing
code <iot/connect/pairing-code>`{.interpreted-text role="ref"}.
::::
...
\- The pairing code is valid for up to 2 hours after the IoT system
powers on. If the code is no longer valid, restart the IoT box or
`restart the Windows virtual IoT service
<iot/windows_iot/restart>`{.interpreted-text role="ref"} to display
another pairing code. Alternatively, connect the IoT system to the
database using a
`connection token <iot/connect/token>`{.interpreted-text role="ref"}. -
The pairing code is not displayed if the IoT system is already connected
to a database (e.g., a test database). If necessary,
`disconnect the IoT system from the database
<iot/connect/disconnect>`{.interpreted-text role="ref"}.
::::
...
1.  Retrieve the IoT system\'s pairing code:
    ...
    Connect the IoT box to an external monitor or a USB printer. If the
    IoT box was already plugged prior to this,
    `restart <iot/iot-box/restart>`{.interpreted-text role="ref"} it.

    - External monitor: The pairing code will be displayed on the screen
      after (re)starting the IoT box.
    - USB-connected printer: The pairing code should be printed
      automatically.
...
3.  In the `Searching for an IoT Box`{.interpreted-text role="guilabel"}
    popup that opens, click `Use Pairing Code`{.interpreted-text
    role="guilabel"}.

4.  Enter the `Pairing code`{.interpreted-text role="guilabel"} and
    click `Connect`{.interpreted-text role="guilabel"}.
...
2.  In Odoo, open the IoT app and click `Connect`{.interpreted-text
    role="guilabel"}.
3.  In the `Searching for an IoT Box`{.interpreted-text role="guilabel"}
    popup that opens, click `Offline pairing`{.interpreted-text
    role="guilabel"}.
4.  In the `Pair an IoT Box offline`{.interpreted-text role="guilabel"}
    popup that opens, copy the `Token`{.interpreted-text
    role="guilabel"}.
5.  Access the `IoT box's <iot/iot-box/homepage>`{.interpreted-text
    role="ref"} or `Windows virtual IoT's
    <iot/windows-iot/homepage>`{.interpreted-text role="ref"} homepage.
6.  In the `fa-link`{.interpreted-text role="icon"}
    `Odoo database connected`{.interpreted-text role="guilabel"}
    section, click `Configure`{.interpreted-text role="guilabel"}.
7.  Paste the token into the `Server Token`{.interpreted-text
...
- If the
  `Point of Sale app </applications/sales/point_of_sale>`{.interpreted-text
  role="doc"} is installed, a popup appears to select the
  `POS the IoT system should be associated with
  </applications/sales/point_of_sale/configuration/pos_iot>`{.interpreted-text
  role="doc"}.
- If using an `IoT box <iot_box>`{.interpreted-text role="doc"}, wait a
  few minutes for it to update before using the connected
  `devices <devices>`{.interpreted-text role="doc"}.
- The IoT system automatically requests an `HTTPS certificate
  <iot/connect/https_certificate>`{.interpreted-text role="ref"} and its
  homepage is updated to a new HTTPS URL ending with
  [.odoo-iot.com]{.title-ref} when it becomes available.
::::

::: seealso
`iot_advanced/troubleshooting`{.interpreted-text role="doc"}
:::

## IoT system form {#iot/connect/IoT-form}

Once the IoT system is connected to the Odoo database, it is displayed
as a card in the IoT app. Click the card to access the IoT system\'s
form, which allows to access the `IoT box's
<iot/iot-box/homepage>`{.interpreted-text role="ref"} or
`Windows virtual IoT's <iot/windows-iot/homepage>`{.interpreted-text
role="ref"} homepage and view the list of
...
role="ref"} to access the IoT system\'s `Technical
Information`{.interpreted-text role="guilabel"}, such as its
`Domain Address`{.interpreted-text role="guilabel"},
`Image Version`{.interpreted-text role="guilabel"}, and
`SSL Certificate <iot/connect/https_certificate>`{.interpreted-text
role="ref"} `End Date`{.interpreted-text role="guilabel"}. By default,
...
## Disconnecting an IoT system from a database {#iot/connect/disconnect}
...
1.  Access the `IoT box's <iot/iot-box/homepage>`{.interpreted-text
    role="ref"} or `Windows virtual IoT's
    <iot/windows-iot/homepage>`{.interpreted-text role="ref"} homepage.
2.  Click `Configure`{.interpreted-text role="guilabel"} in the
    `fa-link`{.interpreted-text role="icon"}
    `Odoo database connected`{.interpreted-text role="guilabel"}
    section.
3.  Click `Disconnect`{.interpreted-text role="guilabel"} in the popup
    that opens.
::::

## HTTPS certificate {#iot/connect/https_certificate}

`HTTPS (Hypertext Transfer Protocol Secure)`{.interpreted-text
role="abbr"} is the secure, encrypted version of `HTTP
(Hypertext Transfer Protocol)`{.interpreted-text role="abbr"}. It uses
`TLS (Transport Layer Security)`{.interpreted-text role="abbr"}
(previously `SSL (Secure Sockets Layer)`{.interpreted-text role="abbr"})
...
- The `HTTPS certificate`{.interpreted-text role="guilabel"} banner
  ...
  (`cogs`{.interpreted-text role="guilabel"}) button at the top-right of
  the IoT system\'s homepage.

  ![IoT box homepage with HTTPS certificate validity date.](connect/https-valid.png)

::: seealso
`iot/troubleshooting/https_certificate`{.interpreted-text role="ref"}
:::
...
`Manufacturing --> Configuration --> Work Centers`{.interpreted-text
...
Triggers are executed in the order they appear, with the first one
taking priority. To reorder the triggers, click the
`oi-draggable`{.interpreted-text role="icon"}
`(draggable)`{.interpreted-text role="guilabel"} icon to the left of
each trigger, and drag it to the desired position.
::::
...
\- To identify the key assigned to each pedal, connect the footswitch to
any computer and press the pedals as if typing. Usually, the pedals are
mapped by default to the keyboard keys [a]{.title-ref}, [b]{.title-ref},
and [c]{.title-ref}. - The `Work Order`{.interpreted-text
role="guilabel"} list includes a status indicator that shows whether the
database is successfully connected to the footswitch.
::::
...
Printers connecting to the IoT system via the network must be on the
**same network** as the IoT system.
::::
...
Printers can take up to two minutes to appear in the IoT app\'s
...
<pos/hardware/scale>`{.interpreted-text role="ref"}.
...
The `IoT box <../iot_box>`{.interpreted-text role="doc"} can be
connected to a screen, e.g. to ease the IoT box\'s setup and/or serve as
a `customer display <pos/hardware/display>`{.interpreted-text
role="ref"} to show customers their `Point
of Sale </applications/sales/point_of_sale>`{.interpreted-text
role="doc"} orders.

## Connection

Connect the screen to the IoT box using a micro-HDMI cable. If the
connection was successful, the external monitor displays the
`Status display`{.interpreted-text role="guilabel"} screen.

![Default Status Display screen](screen/screen-status-display.png)

The screen also appears in the IoT box\'s list of devices:

- on the `IoT box's form <iot/connect/IoT-form>`{.interpreted-text
  role="ref"};
- on the `IoT box's homepage <iot/iot-box/homepage>`{.interpreted-text
  role="ref"};
- in the IoT app, under the menu `IoT --> Devices`{.interpreted-text
  role="menuselection"}.

## Customer display

To use the screen as a
`customer display <pos/hardware/display>`{.interpreted-text role="ref"},
`configure the POS
accordingly <pos/hardware/display-configuration>`{.interpreted-text
role="ref"}.

To display a specific website on the screen instead of the POS customer
display, go to `IoT --> Devices`{.interpreted-text
role="menuselection"}, click the screen\'s card, then enter the desired
website\'s URL in the `Display URL`{.interpreted-text role="guilabel"}
field.
...
devices/printer devices/scale devices/screen devices/measurement_tool
devices/camera devices/footswitch
:::
...
role="doc"} - `Connect a SIX payment terminal
<../../sales/point_of_sale/payment_methods/terminals/six>`{.interpreted-text
role="doc"}
:::
...
1.  Open the IoT app, click the relevant IoT box\'s card, and click
    `Homepage`{.interpreted-text role="guilabel"}.

2.  On the IoT system\'s homepage, click the `fa-cogs`{.interpreted-text
    role="icon"} (`cogs`{.interpreted-text role="guilabel"}) button at
    the top-right, then `Remote Debug`{.interpreted-text
    role="guilabel"}.
...
--- SOURCE: general/iot/iot_advanced/troubleshooting.md ---
...
- The IoT system is already connected to an Odoo database.
  ...
  <iot/connect/disconnect>`{.interpreted-text role="ref"}.
- The `pairing code <iot/connect/pairing-code>`{.interpreted-text
  role="ref"} display time has expired. Restart the `IoT
  box <iot/iot-box/restart>`{.interpreted-text role="ref"} or the
  `Windows virtual IoT service <iot/windows_iot/restart>`{.interpreted-text
  role="ref"} to generate and display the pairing code.
...
- Restart the `IoT box <iot/iot-box/restart>`{.interpreted-text
  role="ref"} or the `Windows virtual IoT service
  <iot/windows_iot/restart>`{.interpreted-text role="ref"}.
...
`update the DNS settings <iot/troubleshooting/dns>`{.interpreted-text
role="ref"} if necessary.

## The Windows virtual IoT\'s homepage cannot be accessed from another device

- Ensure the device is connected to the same network as the Windows
  virtual IoT.
...
`update its image <iot/updating_iot/image-code>`{.interpreted-text
...
role="ref"} and
`reinstalling the virtual IoT package <iot/windows-iot/installation>`{.interpreted-text
role="ref"}.

## The IoT system\'s homepage can be accessed using its IP address but not the [xxx.odoo-iot.com]{.title-ref} URL {#iot/troubleshooting/dns}
...
## HTTPS certificate generation issues and errors {#iot/troubleshooting/https_certificate}

### The HTTPS certificate does not generate

Potential causes include the following:

- The database doesn\'t meet the `eligibility requirements
  <iot/https_certificate_iot/iot-eligibility>`{.interpreted-text
  role="ref"} for generating an `HTTPS certificate
  <iot/connect/https_certificate>`{.interpreted-text role="ref"}.
...
The server configuration is missing, i.e., the database is not
...
`Credentials`{.interpreted-text role="guilabel"}.
...
server refused to provide the ref:\`HTTPS certificate
...
doing so could leave the device in an inconsistent state, requiring the
IoT box to be reflashed with a new image.
::::
:::::::::
...
::: toctree
iot_advanced/troubleshooting iot_advanced/updating_iot
...
1.  Connect your `devices <devices>`{.interpreted-text role="doc"} to
    the IoT box.
2.  `Connect the IoT box to the network <iot/iot_box/network>`{.interpreted-text
    role="ref"}.
3.  `Connect the IoT box to your Odoo database <connect>`{.interpreted-text
    role="doc"}.
...
It is recommended to connect the IoT box to the same network as the
`devices <devices>`{.interpreted-text role="doc"} connecting to it.
::::
...
>     role="guilabel"} section.
>
>     ![IoT box\'s WiFi hotspot](iot_box/iot-wifi.png)
>
...
If the IoT box is connected to an
`external monitor <devices/screen>`{.interpreted-text role="doc"}, you
can also scan the first QR code displayed on the screen to connect to
the network, then scan the second QR code to access the IoT box\'s
`homepage <iot/iot-box/homepage>`{.interpreted-text role="ref"}. On the
homepage, click `Configure`{.interpreted-text role="guilabel"} in the
`Internet Status`{.interpreted-text role="guilabel"} section, select the
relevant Wi-Fi network, enter the password, and click
`Connect`{.interpreted-text role="guilabel"}.
::::

## LED status {#iot/iot-box/led-status}
...
- **Red LED lit**: The IoT box has no Internet connection. Ensure the
  Ethernet cable is properly connected or
  `connect the IoT box via Wi-Fi <iot/iot_box/network-wifi>`{.interpreted-text
  role="ref"}.
- **Green LED flashing**: The IoT box is awaiting
  `connection to a database <connect>`{.interpreted-text role="doc"}.
- **Green LED constantly lit**: The IoT box is connected to a database;
...
- connecting the IoT box to an
  `external monitor <devices/screen>`{.interpreted-text role="doc"}: the
  IP address is displayed on the screen.

  ![Status display with IoT box\'s IP address](iot_box/iot-status-display.png)
...
If the IoT box is connected to an
`external monitor <devices/screen>`{.interpreted-text role="doc"} or a
`printer
<devices/printer>`{.interpreted-text role="doc"}, its homepage can also
be accessed by scanning the displayed or printed QR code with a device
connected to the same network as the IoT box.
::::

Once the IoT box is
`connected to the Odoo database <connect>`{.interpreted-text
role="doc"}, its homepage can be accessed from the IoT app: click the
relevant IoT box\'s card, then click `Homepage`{.interpreted-text
role="guilabel"}.

## IoT box restart {#iot/iot-box/restart}

To manually restart the IoT box, access its
`homepage <iot/iot-box/homepage>`{.interpreted-text role="ref"}, then
click the `fa-power-off`{.interpreted-text role="icon"}
`(power off)`{.interpreted-text role="guilabel"} button in the top-right
corner.
...
You can also fully reboot the IoT box by unplugging and plugging it back
in.
::::
...
1.  Make sure the computer runs an updated version of Windows (Windows
    10 or Windows 11), whether it\'s a laptop, desktop, or server.
2.  `Install the Windows virtual IoT <iot/windows-iot/installation>`{.interpreted-text
    role="ref"} on the Windows computer.
3.  Connect your `devices <devices>`{.interpreted-text role="doc"} to
    the Windows virtual IoT.
4.  `Configure the Windows Firewall <iot/windows-iot/firewall>`{.interpreted-text
    role="ref"}.
...
1.  Access [Odoo\'s download page](https://odoo.com/download), scroll
    down to the `Virtual
    IOT`{.interpreted-text role="guilabel"} section, and click
...
4.  Verify you have the required space on your computer and click
    `Next`{.interpreted-text role="guilabel"}.

5.  In the `Destination folder`{.interpreted-text role="guilabel"},
    ...
    `HTTPS certificate generation <iot/connect/https_certificate>`{.interpreted-text
    role="ref"}.
    ::::

6.  Set up GPL Ghostscript: Click `Next`{.interpreted-text
...
7.  Click `Next`{.interpreted-text role="guilabel"} and
...
8.  `Restart <iot/windows_iot/restart>`{.interpreted-text role="ref"}
    the Windows virtual IoT service.

9.  Check that you can access the
    `IoT system's homepage <iot/windows-iot/homepage>`{.interpreted-text
...
5.  On the `Protocols and Ports`{.interpreted-text role="guilabel"}
    ...
    Other ports may be required depending on your IoT devices. For
    example, add port [9050]{.title-ref} for the
    `/applications/sales/point_of_sale/payment_methods/terminals/worldline`{.interpreted-text
    role="doc"} payment terminal and port [9000]{.title-ref} for the
    `/applications/sales/point_of_sale/payment_methods/terminals/ingenico`{.interpreted-text
    role="doc"} payment terminal.
    ::::
...
app, clicking the relevant IoT box\'s card, and clicking
`Homepage`{.interpreted-text role="guilabel"}.
...
menu for [services]{.title-ref} and select the Services app. Scroll down
to the `odoo-server-xxx`{.interpreted-text role="guilabel"} service,
right-click it, and select `Start`{.interpreted-text role="guilabel"} or
...
\- `MRP (Material Requirement Planning)`{.interpreted-text role="abbr"}
devices, including cameras and measurement tools, are not compatible
with the Windows virtual IoT. - Multiple IoT systems can be used at the
same time. - It is also possible to create a Windows Virtual Machine on
a MacOS/Linux computer. However, this option is not supported by Odoo,
and no troubleshooting assistance will be provided.
::::
...
and generating the
`HTTPS certificate <iot/connect/https_certificate>`{.interpreted-text
role="ref"}. It is created automatically when an IoT system is
`connected to the database <iot/connect>`{.interpreted-text role="doc"}.
...
::: {.card target="iot/iot_advanced/troubleshooting"}
Troubleshooting

Diagnose and resolve common IoT system connection and configuration
issues.
:::
...
*Two-factor authentication (2FA)* is a security measure that helps
prevent unauthorized access to user accounts.

Practically, `2FA (two-factor authentication)`{.interpreted-text
role="abbr"} means storing a secret in an *authenticator*, usually on a
mobile phone, and exchanging a code from the authenticator when logging
in.
...
![The account secuirty tab in a user profile.](2fa/account-security.png)
...
`QR (Quick Response)`{.interpreted-text role="abbr"} code when
prompted.](2fa/qr-code.png){alt="The 2fa authentication QR code in Odoo."}
...
completed on the *same* device as the authenticator application), click
the provided `Cannot scan it?`{.interpreted-text role="guilabel"} link,
or copying the code to set up the authenticator manually, is an
alternative.

![A 2fa secret code on an authentication popup.](2fa/secret-visible.png)
::::

Afterwards, the authenticator should display a *verification code*.

![A view of the Google authenticator app with the six digit code for 2fa.](2fa/authenticator.png)

Enter the code into the `Verification Code`{.interpreted-text
role="guilabel"} field, then click `Activate`{.interpreted-text
role="guilabel"}.

![The success message that appears in a user profile when 2fa is successfully enabled.](2fa/2fa-enabled.png)
...
To enforce `2FA (two-factor authentication)`{.interpreted-text
role="abbr"} for every user in the database, navigate to the
`Settings`{.interpreted-text role="menuselection"} app. In the
`Permissions`{.interpreted-text role="guilabel"} section, tick the
checkbox labeled `Enforce two-factor
authentication`{.interpreted-text role="guilabel"}. Then, use the radio
buttons to choose whether to apply this setting to
`Employees only`{.interpreted-text role="guilabel"} or
...
## Manage user permissions {#access-rights/user-permissions}
...
### Manage specific permissions {#access-rights/specific-user-permissions}

While access rights are typically assigned in bundles under specific
roles, they can also be set as explicit permissions.

::: example
For example, giving a user the `Administrator`{.interpreted-text
role="guilabel"} permission for **Timesheets** gives them full access to
that app. That user, while holding full access, can *still* have their
ability to manage *their own* timesheets restricted --- such as in the
case of a salaried payroll administrator who does not need to track
time.
:::
...
After that, navigate to the `Settings`{.interpreted-text
role="menuselection"} app. Then click `Manage Users`{.interpreted-text
role="guilabel"}, select a user, and go to the
`Technical Access Rights`{.interpreted-text role="guilabel"} tab. From
here, `Groups`{.interpreted-text role="guilabel"} can be edited, and
specific access rights can be managed across the various sections. If no
changes are made to these groups, then their permissions will mirror the
selections made in the `Access Rights`{.interpreted-text
role="guilabel"} tab.

- `Selected groups`{.interpreted-text role="guilabel"}: a list of
  detailed access rights, set by choices made in the
  `Access Rights`{.interpreted-text role="guilabel"} tab.
- `Groups added automatically`{.interpreted-text role="guilabel"}:
  *implied* permissions that are *inherited* with the explicit
  permissions already granted to the user. The values here will match
  the values listed under a given *Group*\'s form located under the
  `Users & Companies --> Groups`{.interpreted-text role="menuselection"}
  menu, in the `Inherited`{.interpreted-text role="guilabel"} tab.

![The technical access rights tab opened up for a user profile.](access_rights/tech-access-rights.png)

::: example
When the *Sales Administrator* permission set is assigned to a user,
then the *Canned Responses Administrator* permissions are inherited
automatically. These assignments are reflected across the values listed
in the `Selected Groups`{.interpreted-text role="guilabel"} and
`Groups added automatically`{.interpreted-text role="guilabel"} tables,
respectively.
:::

To add a permission to this user profile, click
`Add a line`{.interpreted-text role="guilabel"} in the `Selected
groups`{.interpreted-text role="guilabel"} table, and then add
permissions to this user profile. To remove a permission, click the
`fa-times`{.interpreted-text role="icon"} `(cancel)`{.interpreted-text
role="guilabel"} at the end of that permission\'s row.
...
Removing permissions from the `Selected Groups`{.interpreted-text
role="guilabel"} list can impact what permissions are listed in the
`Groups added automatically`{.interpreted-text role="guilabel"} list,
since selected permission groups inform what permission groups are added
automatically.
::::

Clicking on the permission itself will open a group management form.
Learn more about `managing
groups <access-rights/groups>`{.interpreted-text role="ref"}.

Any permission in the `Groups added automatically`{.interpreted-text
role="guilabel"} section are implied or required by the permission shown
in the `Selected groups`{.interpreted-text role="guilabel"} section.
These cannot be removed, but more users can be given these permissions
by clicking on the permission itself, and then adding the user to that
permission\'s group.
...
\- Any permission in green is already provided by another permission
(for example, setting the `Website`{.interpreted-text role="guilabel"}
app\'s permission to `Editor and Designer`{.interpreted-text
role="guilabel"} will also give that user the
`Restricted Editor`{.interpreted-text role="guilabel"} permission). -
Any permissions in red are conflicting and cannot be active at the same
time. - Any permissions in *italics* is implied by a
`Selected group`{.interpreted-text role="guilabel"} (these are usually
found in the `Groups added automatically`{.interpreted-text
role="guilabel"}).
::::
...
Click `New`{.interpreted-text role="guilabel"} and in the blank row that
appears, add the following system parameter
...
Odoo needs permission to read the signed-in user\'s profile information
from Microsoft Graph during the OAuth flow. At minimum, ensure the
delegated permission `User.Read`{.interpreted-text role="guilabel"} is
granted on the Azure app registration. Otherwise, the login process may
fail or be unable to correctly link the Microsoft account to the Odoo
user.
...
`User.Read`{.interpreted-text role="guilabel"} is often added by default
for new app registrations, but it should be explicitly verified here.
::::

To configure this, click on the `Manage`{.interpreted-text
role="guilabel"} menu item in the left-hand column, then:

1.  Click `API permissions`{.interpreted-text role="guilabel"} in the
    left menu.
2.  Click `(+) Add a Permission`{.interpreted-text role="guilabel"}.
3.  Select `Microsoft Graph`{.interpreted-text role="guilabel"} under
    `Commonly Used Microsoft APIs`{.interpreted-text role="guilabel"}.
4.  Choose `Delegated Permissions`{.interpreted-text role="guilabel"}.
5.  Search for `User.Read`{.interpreted-text role="guilabel"}, select
    it, and click `Add permissions`{.interpreted-text role="guilabel"}.

### Odoo setup

Finally, the last step in the Microsoft Azure OAuth configuration is to
complete the setup in Odoo. Navigate to
`Settings --> Integrations`{.interpreted-text role="menuselection"} and
enable the checkbox for `OAuth
Authentication`{.interpreted-text role="guilabel"}. Click
`Save`{.interpreted-text role="guilabel"} to save the changes and
refresh the page. Scroll down to the `Integrations`{.interpreted-text
role="menuselection"} section, and click on
`OAuth Providers`{.interpreted-text role="guilabel"}. Then click
`New`{.interpreted-text role="guilabel"}.

Enter the `Provider name`{.interpreted-text role="guilabel"} as
[Azure]{.title-ref}. Paste the
`Application (client) ID`{.interpreted-text role="guilabel"} from the
previous section into the `Client ID`{.interpreted-text role="guilabel"}
field. After completing this, paste the new
`OAuth 2.0 authorization endpoint (v2)`{.interpreted-text
...
--- SOURCE: general/users/user_portals/portal_access.md ---

# Granting portal access

For the customers and vendors of a business to begin using a user
portal, they must first be granted access by the business. The process
to do this begins in the **Contacts** application.

## Adding portal access to a contact

From the main Odoo dashboard, open the `Contacts`{.interpreted-text
role="guilabel"} application. If the contact to be granted access is not
yet in the database, create an entry for them by clicking the
`New`{.interpreted-text role="guilabel"} button and entering their
details. Otherwise, choose an existing contact, then click on the
`fa-gear`{.interpreted-text role="icon"} `(Actions)`{.interpreted-text
role="guilabel"} drop-down menu and select
`Grant portal access`{.interpreted-text role="guilabel"}.

![Use the Contacts application to give portal access to users.](portal_access/grant-portal-access.png)

This brings up the `Portal Access Management`{.interpreted-text
role="guilabel"} window, which has three fields:

- `Contact`{.interpreted-text role="guilabel"}: This field is
  automatically populated with the name on the contact.
- `Email`{.interpreted-text role="guilabel"}: This field shows the
  contact\'s email address used to log into the portal.
- `Latest Authentication`{.interpreted-text role="guilabel"}: The last
  time the user accessed the portal appears on this line. If the user
  has never accessed the portal, it is blank.

To grant portal access, first enter the email address the contact will
use to log into the portal. This may have been automatically entered by
the system if an existing contact is being granted access. Then click
the `Grant Access`{.interpreted-text role="guilabel"} button to finish.

![Grant access in the Portal Access Management window.](portal_access/portal-access-management.png)
...
navigate to the contact listing for that company, then click
`Action --> Grant portal access`{.interpreted-text role="menuselection"}
to view a list of the company\'s related contacts. Click
`Grant Access`{.interpreted-text role="guilabel"} for each contact that
needs portal access. An email is sent to the specified email address,
indicating that the contact is now a portal user for that Odoo database.

![Grant access to multiple users at once.](portal_access/multiple-user-access.png)

### Revoking portal access

Some situations may require revoking an individual\'s portal access,
such as when they retire or leave a company. Portal access can be
revoked from a contact at any time by navigating to a contact, clicking
`Action --> Grant portal access`{.interpreted-text
role="menuselection"}, and then clicking `Revoke
Access`{.interpreted-text role="guilabel"}.

![Revoke access from a user.](portal_access/revoke-user-access.png)




--- SOURCE: general/users/user_portals/updating_portal_info.md ---

# Updating portal info

There may be times when a user would like to make changes to their
contact, security, or payment information connected to their portal
account. Portal users can update most information by logging into their
account and making the changes themselves, but some updates may require
a user with administrator access rights.

Updating user information begins with logging into the user portal. Once
a user has logged in and is on the portal dashboard, they can begin
making changes to their information.
...
From the portal dashboard, click the `fa-pencil`{.interpreted-text
role="icon"} `Edit information`{.interpreted-text role="guilabel"}
button under the user contact information. This brings up a new page
where contact information can be updated.

On this screen, users can update their:

- `Your name`{.interpreted-text role="guilabel"}
- `Email`{.interpreted-text role="guilabel"}
...
Once users have updated their information, they can click
`Save Address`{.interpreted-text role="guilabel"} to save any changes
they\'ve made or `Discard`{.interpreted-text role="guilabel"} to return
to the portal dashboard. By default, the only fields that need to be
filled out to save users\' information are their names, emails, phone
...
country **cannot** be updated. If users need to update their country at
that point, they must reach out to a user with administrator access
rights.
::::

### Update payment info {#users-portal-payment-methods}

From the portal dashboard, click the `Payment methods`{.interpreted-text
role="guilabel"} button. This brings up a new page that shows any saved
payment methods.

To add a new payment option, choose from the available payment methods,
enter the new payment information, and click `Save`{.interpreted-text
role="guilabel"} to save the details. To remove a payment option, click
the `fa-trash`{.interpreted-text role="icon"}
`(Delete payment method)`{.interpreted-text role="guilabel"} icon, then
click the `Confirm
Deletion`{.interpreted-text role="guilabel"} button.
...
Users that need to update payment information **must** delete any
existing information and enter their new or updated info. Payment
information **cannot** be edited and must be re-entered.
::::

### Update passwords and manage security

From the portal dashboard, click the
`Connection & Security`{.interpreted-text role="guilabel"} button. This
brings up a new page with five sections. The following sections can each
be edited and updated.

#### Change Password

Here the user can enter their current password and a new password they
wish to change to. The new password can be saved by clicking
`Change Password`{.interpreted-text role="guilabel"}.

#### Two-factor authentication

The user\'s currrent 2FA status is shownn as an alert. If 2FA is not
enabled, an `Enable
two-factor authentication`{.interpreted-text role="guilabel"} link
appears. If it is enabled, a link reading `Disable
two-factor authentication`{.interpreted-text role="guilabel"} appears
instead.
...
Clicking the `fa-info-circle`{.interpreted-text role="icon"}
`(Documentation)`{.interpreted-text role="guilabel"} icon opens a page
that explains how two-factor authentication (2FA) works.
::::

#### Passkeys

Click `Add Passkey`{.interpreted-text role="guilabel"} to create a
passkey that simplifies the login process. Any previously created
passkeys are shown here, alongside information on when they were created
and when they were last used. When creating a passkey, Odoo asks for a
password to confirm account ownership before allowing a passkey to be
set. Once account ownership has been confirmed, the passkey must be
named to differentiate it from other passkeys. Clicking
`Create`{.interpreted-text role="guilabel"} brings up a browser prompt
to create the passkey. Complete the passkey creation process in the
browser. They can be renamed by clicking the
`fa-pencil`{.interpreted-text role="icon"} `(pencil)`{.interpreted-text
role="guilabel"} button or deleted by clicking their
`fa-trash`{.interpreted-text role="icon"}
`(trash can)`{.interpreted-text role="guilabel"} button.

#### Revoke All Sessions

Clicking the `Log out from all devices`{.interpreted-text
role="guilabel"} button brings up a pop-up prompting the user to enter
their password. Once they do so and click
`Confirm Password`{.interpreted-text role="guilabel"}, the system logs
them out of all sessions except for the current one.

#### Delete Account

Clicking the `Delete Account`{.interpreted-text role="guilabel"} button
brings up a pop-up to completely and irrevocably delete the user\'s
account. Users must enter their password and their login name to
complete the process. They can also choose to have their email address
and phone number added to an internal block list to prevent any future
communication.
...
role="menuselection"} and click `oi-arrow-right`{.interpreted-text
role="icon"} `Manage Users`{.interpreted-text role="guilabel"} Then,
click the `fa-times`{.interpreted-text role="icon"}
`(Remove)`{.interpreted-text role="guilabel"} icon to clear the
`Internal Users`{.interpreted-text role="guilabel"} search filter and
use the `fa-sort-desc`{.interpreted-text role="icon"} `(Toggle Search
Panel)`{.interpreted-text role="guilabel"} button to add
`Portal Users`{.interpreted-text role="guilabel"} as a filter. After
making this selection, search for and open the portal user that needs to
be edited.

#### Update email address

The `fa-envelope`{.interpreted-text role="icon"}
`(Email)`{.interpreted-text role="guilabel"} field shows the email
address the user uses to log into the Odoo database. A user with
administrator access rights can make any necessary changes by clicking
into the field and updating the user\'s email address.
...
Users **cannot** change their own login usernames. Changing the
`fa-key`{.interpreted-text role="icon"} `(Login)`{.interpreted-text
role="guilabel"} only changes the *username* on the user\'s portal
login. Users\' email addresses can be changed in the
`Manage Users`{.interpreted-text role="guilabel"} module, the
**Contacts** app, or by the users themselves in the portal.
::::

#### Update passwords

Users\' security settings can be adjusted under the
`Security`{.interpreted-text role="guilabel"} tab on their Contact form.
Under the `Security`{.interpreted-text role="guilabel"} tab, there are
options to:

- `Change password`{.interpreted-text role="guilabel"} Click the
  `Change password`{.interpreted-text role="guilabel"} button to bring
  up a form that shows what logins the user has access to and enter a
  new password. Click `Change
  Password`{.interpreted-text role="guilabel"} to save the password or
  click `Cancel`{.interpreted-text role="guilabel"} to return to the
  previous screen.
- `Invite to use 2FA`{.interpreted-text role="guilabel"}. Click the
  `Invite to use 2FA`{.interpreted-text role="guilabel"} button to bring
  up a pop-up notification that an invitation to use two-factor
  authentication was sent.

::: seealso
\-
`Access rights </applications/general/users/access_rights>`{.interpreted-text
role="doc"} -
`Portal access documentation <portal_access>`{.interpreted-text
role="doc"}
:::
...
--- SOURCE: general/users/user_portals.md ---
...
customers and vendors, can be granted
`access to the portal </applications/general/users/access_rights>`{.interpreted-text
role="doc"} by businesses in order to view certain documents or
information within an Odoo database. For example, some common use cases
for providing portal access include allowing customers and vendors to:
:::
...
Portal users only have read/view access, and will **not** be able to
edit any documents in the database.
::::

::: {.toctree titlesonly=""}
user_portals/portal_access user_portals/updating_portal_info
:::
...
documentation on `users/user_portals`{.interpreted-text role="doc"}.
...
::: {.toctree titlesonly=""}
users/language users/2fa users/access_rights users/user_portals
...
3.  Set the `Target Record`{.interpreted-text role="guilabel"} to
    [model.env\[payload.get(\'\_model\')\].browse(int(payload.get(\'\_id\')))]{.title-ref},
    where:
    - [payload.get(\'\_model\')]{.title-ref} retrieves the value
      associated with the [model]{.title-ref} key in the payload, i.e.,
      [sale.order]{.title-ref}, which is the technical name of the
      *Sales Order* model.
    - [payload.get(\'\_id\')]{.title-ref} retrieves the value associated
...
When creating an automation rule, it is possible to
`add conditions that must be met
<studio/automated-actions/conditions>`{.interpreted-text role="ref"} for
the automation rule to run, e.g., the opportunity must be assigned to a
...
4.  Click
    `Add an action <studio/automated-actions/action>`{.interpreted-text
    role="ref"} in the `Actions To Do`{.interpreted-text
    role="guilabel"} tab.
5.  Select the `Type`{.interpreted-text role="guilabel"} of action and
    complete the relevant fields based on the chosen action.
6.  Click `Save & Close`{.interpreted-text role="guilabel"} or, to
    define additional actions, `Save & New`{.interpreted-text
    role="guilabel"}.
...
![Example of an automation rule on the Subscription model](automated_actions/trigger-timing-conditions.png)
:::
...
- The `Before Update Domain`{.interpreted-text role="guilabel"} defines
  ...
  With developer mode activated, click `Edit Domain`{.interpreted-text
  role="guilabel"}, if available, then `New Rule`{.interpreted-text
  role="guilabel"}.

- The `Apply on`{.interpreted-text role="guilabel"} domain defines the
  conditions a record must meet *after* the trigger event occurs, e.g.,
  the customer invoice must have [Payment Status = Partially
  Paid]{.title-ref}.

  With developer mode activated, click `Edit Domain`{.interpreted-text
  role="guilabel"} then `New Rule`{.interpreted-text role="guilabel"}.
  ...
  If developer mode is not activated, to define an
  `Apply on`{.interpreted-text role="guilabel"} domain, click
  `fa-plus`{.interpreted-text role="icon"}
  `Add condition`{.interpreted-text role="guilabel"}. Complete or delete
  the suggested condition. To add a condition, click
  `New Rule`{.interpreted-text role="guilabel"}.
  ::::
...
- `Based on date field`{.interpreted-text role="guilabel"}: The action
  is triggered a defined period of time *before or after* the date of
  the selected date field, e.g., 30 days before the contract end date.
...
To configure a `Timing Conditions`{.interpreted-text role="guilabel"}
trigger, with `developer mode activated
<developer-mode>`{.interpreted-text role="ref"}:

1.  Select the appropriate `Trigger`{.interpreted-text role="guilabel"}
    from the `Timing Conditions`{.interpreted-text role="guilabel"}
    category. If you select the `Based on date field`{.interpreted-text
    role="guilabel"} trigger, select the relevant date field.

2.  Configure the `Delay`{.interpreted-text role="guilabel"} to
    determine the *execution date time* of the action(s). To do so,
    enter the number of `Minutes`{.interpreted-text role="guilabel"},
    `Hours`{.interpreted-text role="guilabel"}, `Days`{.interpreted-text
    role="guilabel"}, or `Months`{.interpreted-text role="guilabel"}
    after which the action should be triggered. If you selected the
    `Based on date field`{.interpreted-text role="guilabel"} trigger,
    the action can be triggered `After`{.interpreted-text
    role="guilabel"} or `Before`{.interpreted-text role="guilabel"} the
    selected date field.

3.  Optionally,
    `add conditions <studio/automated-actions/conditions>`{.interpreted-text
    role="ref"} that the record must meet before and/or after the rule
    is triggered.

4.  Click `fa-cloud-upload`{.interpreted-text role="icon"}
    `Save manually`{.interpreted-text role="guilabel"}.

5.  Click `fa-arrow-right`{.interpreted-text role="icon"}
    `Scheduled action`{.interpreted-text role="guilabel"} to open the
    `Automation
    Rules: check and execute`{.interpreted-text role="guilabel"}
    scheduled action.
    ...
    This scheduled action checks for execution date times that have been
    reached since its previous run, and executes the related action(s),
    if relevant. By default, the scheduled action is set to
    `Execute Every`{.interpreted-text role="guilabel"} 4 hours, which is
    generally sufficient for delays such as 3 months after the order
    date or 7 days after the last update.

    If any automation rule has a time-based trigger with a delay of less
    than the equivalent of 40 hours, e.g., 3 hours before the event
    start date time, the frequency of the scheduled action is
    automatically increased, e.g., to `Execute Every`{.interpreted-text
    role="guilabel"} 18 minutes. This allows the action to be executed
    closer to its scheduled execution date time.
    ::::

6.  Click `Run manually`{.interpreted-text role="guilabel"} at the top
    left.
    ...
    This step is critical to prevent the rule from running retroactively
    on historical records. Running the scheduled action manually
    initializes the timestamp of its *last run* in the automation rule,
    which is a key element of the rule\'s `execution logic
    <studio/automated-actions/trigger-timing-conditions-logic>`{.interpreted-text
    role="ref"}.
    ::::

7.  Click the automation rule name in the breadcrumbs to return to the
    automation rule setup.

8.  Add the
    `action(s) to be executed <studio/automated-actions/action>`{.interpreted-text
    role="ref"} then, when all actions have been defined, click
    `fa-cloud-upload`{.interpreted-text role="icon"}
    `Save manually`{.interpreted-text role="guilabel"}.

The action is executed the first time the
`Automation Rules: check and execute`{.interpreted-text role="guilabel"}
scheduled action runs after the execution date time is reached, and when
the conditions are met.

#### Execution logic {#studio/automated-actions/trigger-timing-conditions-logic}

When using a time-based trigger, an action is only executed when the
execution date time falls *between* the last run of the scheduled action
and the current run (whether scheduled or run manually), i.e.:
...
Running the `Automation Rules: check and execute`{.interpreted-text
role="guilabel"} scheduled action manually does not necessarily result
in a rule\'s action being executed. When testing an automation rule with
a time-based trigger, ensure that this sequence is true, then check if
the action has been executed.
::::

::: example
On the morning of October 15, you create an automation rule that
triggers the sending of an email 30 days before the
`Contract end date`{.interpreted-text role="guilabel"}; contracts end at
11:59 pm on their contract end date. During the creating of the rule,
the `Automation Rules: check and execute`{.interpreted-text
role="guilabel"} scheduled action was run manually at 11:00 am and is
set to run every four hours thereafter.

This new automation rule will apply for contracts whose end date is at
least 30 days *after* the creation of the rule, i.e., an end date of
November 14 or later.

For a contract with an end date of November 14, the execution date time
of the action is 11:59 pm on October 15. The action will therefore be
executed the first time the `Automation
Rules: check and execute`{.interpreted-text role="guilabel"} scheduled
...
- `On create`{.interpreted-text role="guilabel"}: when a record is saved
  for the first time.
- `On create and edit`{.interpreted-text role="guilabel"}: when a record
  is saved for the first time and any subsequent time.
...
For the `On create and edit`{.interpreted-text role="guilabel"} and
...
\- If no explicit name is entered, the name of the action will be
automatically generated based on the action you define; the name can be
updated at any time. - You can define multiple actions for the same
automation rule. By default, actions are executed in the order in which
they were defined.
...
- `Update`{.interpreted-text role="guilabel"}: updates the selected
  field with the specified value.
- `Update with AI`{.interpreted-text role="guilabel"}: updates the
  selected field dynamically based on the AI prompt provided. This
  option requires the **Odoo AI** app to be installed.
- `Sequence`{.interpreted-text role="guilabel"}: update the selected
  field with a defined sequence.
- `Compute`{.interpreted-text role="guilabel"}: updates the selected
  field dynamically using Python code.

To define the action:

1.  With the `Update`{.interpreted-text role="guilabel"},
    `Update with AI`{.interpreted-text role="guilabel"},
    `Sequence`{.interpreted-text role="guilabel"} or
    `Compute`{.interpreted-text role="guilabel"} option selected, as
    relevant, select or search for the field to be updated or computed.
    If needed, click the `oi-chevron-right`{.interpreted-text
    role="icon"} `(right arrow)`{.interpreted-text role="guilabel"} next
    to the field name to access the list of related fields.
...
role="ref"} is being updated, choose whether the field must be updated
by `Adding`{.interpreted-text role="guilabel"},
`Removing`{.interpreted-text role="guilabel"}, or `Setting
it to`{.interpreted-text role="guilabel"} the selected value or by
`Clearing it`{.interpreted-text role="guilabel"}.
...
`by Removing`{.interpreted-text role="guilabel"}, then select the tag to
be removed.
...
[/]{.title-ref} to open the `AI
prompt tools`{.interpreted-text role="guilabel"}; use
`Field Selector`{.interpreted-text role="guilabel"} to tell Odoo AI
which related fields to check for context, and
`Records Selector`{.interpreted-text role="guilabel"} to provide
possible values for the updated field.

::: example
If you want the automation rule to update the
`Assignees`{.interpreted-text role="guilabel"} field of a newly created
task based on the employees\' expertise, you can write a prompt to
instruct Odoo AI to check the `Display Name`{.interpreted-text
role="guilabel"} field of the task for the context, then to assign the
most suitable employee.

![Example of updating a record using AI](automated_actions/update-record-ai.png)
:::
...
1.  Click `Search more`{.interpreted-text role="guilabel"}, then click
    `New`{.interpreted-text role="guilabel"}.

2.  In the window that opens, enter the `Name`{.interpreted-text
    role="guilabel"} of the sequence.

3.  In the `Sequence`{.interpreted-text role="guilabel"} tab, configure
    the sequence:

    - `Prefix`{.interpreted-text role="guilabel"}: characters added
      *before* the next number in the sequence.
    - `Suffix`{.interpreted-text role="guilabel"}: characters added
      *after* the next number in the sequence.
    - `Sequence Size`{.interpreted-text role="guilabel"}: determines the
      number of digits in every number in the sequence. If needed,
      leading zeros are added before the number to achieve the indicated
      sequence size, e.g., for a sequence size of [5]{.title-ref}, the
      first number in the sequence is [00001]{.title-ref}.
    - `Step`{.interpreted-text role="guilabel"}: determines the
      increments between the numbers in the sequence.
    - `Next Number`{.interpreted-text role="guilabel"}: the next number
      that will be used in the sequence, without leading zeros.
    ...
    \- Use dynamic placeholders like [%(year)s]{.title-ref} or
    [%(month)s]{.title-ref} as a `Prefix`{.interpreted-text
    role="guilabel"} and/or `Suffix`{.interpreted-text role="guilabel"}
    to create sequences with elements like current year, month, etc.
    Possible placeholders are shown at the bottom of the window.
    \- To use sub-sequences, e.g., to have the sequence restart each
    year or each month, enable
    `Use subsequences per date_range`{.interpreted-text
    role="guilabel"}, then `Add a line`{.interpreted-text
    role="guilabel"} for each date range and indicate the
    `Next Number`{.interpreted-text role="guilabel"} for the range.
    ::::

4.  Click `Save`{.interpreted-text role="guilabel"}.

::: example
If you want the automated action to create a sequential customer
reference each time a new customer is created, set the
`Sequence`{.interpreted-text role="guilabel"} field to
`Reference`{.interpreted-text role="guilabel"}, then, in the dropdown,
click `Search more`{.interpreted-text role="guilabel"}. Click
`New`{.interpreted-text role="guilabel"} to create a new sequence.

In the example, each new customer receives a sequential reference with
the prefix [#-REF-%(year)s-]{.title-ref}, where [%(year)s]{.title-ref}
is the current year including the century, and the suffix
[/CL]{.title-ref}, e.g., [#-REF-2025-00001/CL]{.title-ref},
[#-REF-2025-00002/CL]{.title-ref}, etc.

![Example of an Update Record action using a sequence](automated_actions/update-record-sequence.png)
:::
...
::: example
If you want the automation rule to compute a custom `datetime field
<studio/fields/simple-fields-date-time>`{.interpreted-text role="ref"},
`Escalated on`{.interpreted-text role="guilabel"}, when a task\'s
priority is set to [Very high]{.title-ref} (three stars), you can define
the trigger `Priority is set to`{.interpreted-text role="guilabel"} to
[Very High]{.title-ref} and define the `Update Record`{.interpreted-text
...
### Create Record and Duplicate Record {#studio/automated-actions/action-create-record}

These actions are used to create a new or duplicated record on any
model.

To define the action:

1.  With `Create Record`{.interpreted-text role="guilabel"} or
    `Duplicate Record`{.interpreted-text role="guilabel"} selected as
    the `Type`{.interpreted-text role="guilabel"} of action, as
    relevant, select the required model in the
    `Record to Create`{.interpreted-text role="guilabel"} field; the
    field contains the current model by default.
2.  Specify a `Name`{.interpreted-text role="guilabel"} for the record,
    or, if the action duplicates a record, indicate the record being
    duplicated in the `Duplicate from`{.interpreted-text
    role="guilabel"} field.
3.  If the new or duplicated record is being created on another model,
    select a field in the `Link Field`{.interpreted-text
    role="guilabel"} field to link the record that triggered the
    creation of the new or duplicated record.
...
The `Link Field`{.interpreted-text role="guilabel"} dropdown menu only
contains `one2many fields
...
template with predefined tasks, when an opportunity is set to
[Won]{.title-ref}, add a custom [Related opportunity]{.title-ref}
`many2one
field <studio/fields/relational-fields-many2one>`{.interpreted-text
role="ref"} field on the *Project* model and a custom [Related
project]{.title-ref}
`one2many field <studio/fields/relational-fields-one2many>`{.interpreted-text
role="ref"} field on the *Lead* model, then provide the following
details about the record being duplicated:

![Duplicate a record in a different model](automated_actions/duplicate-record.png)
:::
...
role="ref"} actions to update the fields of the new or duplicated record
if necessary. For example, you can use a
`Create Record`{.interpreted-text role="guilabel"} action to create a
new project task and then assign it to a specific user using an
`Update Record`{.interpreted-text role="guilabel"} action.
::::
...
1.  With `Create Activity`{.interpreted-text role="guilabel"} selected
    as the `Type`{.interpreted-text role="guilabel"} of action, select
    the appropriate `Activity Type`{.interpreted-text role="guilabel"}
    from the dropdown menu.
2.  Enter a `Title`{.interpreted-text role="guilabel"}.
3.  Indicate by when the activity should be completed by specifying a
    number of `Days`{.interpreted-text role="guilabel"},
    `Weeks`{.interpreted-text role="guilabel"}, or
    `Months`{.interpreted-text role="guilabel"} in the
    `Due Date In`{.interpreted-text role="guilabel"} field.
4.  Select a `User type`{.interpreted-text role="guilabel"}:
    - To always assign the activity to the same user, select
      `Specific User`{.interpreted-text role="guilabel"}, then add the
      user in the `Responsible`{.interpreted-text role="guilabel"}
      field.
    - To target a user linked to the record dynamically, select
      `Dynamic User (based on
      record)`{.interpreted-text role="guilabel"}. If needed, change the
      `User Field`{.interpreted-text role="guilabel"} by clicking on the
      placeholder field name then selecting or searching for the user
      field in the list that appears. Clicking the
      `oi-chevron-right`{.interpreted-text role="icon"}
      `(right arrow)`{.interpreted-text role="guilabel"} next to the
      field name allows you to access related fields if needed.
5.  Optionally, add a `Note`{.interpreted-text role="guilabel"} to
    provide more information about the activity.

::: example
After a proposition is sent to a opportunity with a high expected
revenue, you want to create an activity for the salesperson\'s team
leader to call the potential client to increase the chances of closing
the deal.

To do so, set the `Activity Type`{.interpreted-text role="guilabel"} to
...
Click on the placeholder field and select `Sales Team`{.interpreted-text
role="guilabel"}, then click the `oi-chevron-right`{.interpreted-text
role="icon"} `(right arrow)`{.interpreted-text role="guilabel"} and
select `Team Leader`{.interpreted-text role="guilabel"}.
...
1.  With `Send Email`{.interpreted-text role="guilabel"} or
    `Send SMS`{.interpreted-text role="guilabel"} selected as the
    `Type`{.interpreted-text role="guilabel"} of action, as relevant,
    select or create an `Email Template`{.interpreted-text
    role="guilabel"} or `SMS Template`{.interpreted-text
    role="guilabel"}.

2.  In the `Send Email As`{.interpreted-text role="guilabel"} or
    `Send SMS As`{.interpreted-text role="guilabel"} field, choose how
    you want to send the email or text message.
    ...
    - `Message`{.interpreted-text role="guilabel"}: to post the message
      on the record and notify the record\'s followers.
    - `Note`{.interpreted-text role="guilabel"}: to send the message as
      an internal note visible to internal users in the chatter.

    For a text message, select:

    - `SMS (without note)`{.interpreted-text role="guilabel"}: to send
      the message as a text message to the recipients of the
      `SMS template`{.interpreted-text role="guilabel"}.
    - `SMS (with note)`{.interpreted-text role="guilabel"}: to send the
      message as a text message to the recipients of the
      `SMS template`{.interpreted-text role="guilabel"} and post it as
      an internal note in the chatter.
...
With `Send WhatsApp`{.interpreted-text role="guilabel"} selected as the
`Type`{.interpreted-text role="guilabel"} of action, select the
appropriate `WhatsApp Template`{.interpreted-text role="guilabel"} from
the dropdown menu.

### Add Followers and Remove Followers {#studio/automated-actions/action-add-remove-followers}

This action is used to subscribe/unsubscribe existing contacts as
followers of the record.

With `Add Followers`{.interpreted-text role="guilabel"} or
`Remove Followers`{.interpreted-text role="guilabel"} selected as the
`Type`{.interpreted-text role="guilabel"} of action, as relevant, select
a `Followers Type`{.interpreted-text role="guilabel"}:

- To always add/remove the same contact(s), select
  `Specific Followers`{.interpreted-text role="guilabel"} then select
  the contact(s) from the dropdown menu. Multiple contacts can be
  added/removed.
- To add/remove a contact linked to the record dynamically, select
  `Dynamic Followers`{.interpreted-text role="guilabel"}. If needed,
  change the `Followers Field`{.interpreted-text role="guilabel"} by
  clicking on the placeholder field name then selecting or searching for
  the partner field in the list that appears. Clicking the
  `oi-chevron-right`{.interpreted-text role="icon"}
  `(right arrow)`{.interpreted-text role="guilabel"} next to the field
  name allows you to access related fields if needed.

::: example
To keep customers informed of progress on a project, this automated
action adds the relevant customer as a follower when a project task is
set to [In progress]{.title-ref}.

![Adding the customer as a follower when project task set to in progress](automated_actions/add-followers-action.png)
:::
...
### Multi Actions {#studio/automated-actions/action-existing-actions}
...
1.  With `Multi Actions`{.interpreted-text role="guilabel"} selected as
    the `Type`{.interpreted-text role="guilabel"} of action, click `Add
    an action`{.interpreted-text role="guilabel"}.
2.  In the `Add: Child Actions`{.interpreted-text role="guilabel"}
    pop-up:
    - select one or more existing actions and click
      `Select`{.interpreted-text role="guilabel"}; or
    - click `New`{.interpreted-text role="guilabel"}, define the action
      to be executed, then click `Save & Close`{.interpreted-text
      role="guilabel"} or, to create additional actions,
      `Save & New`{.interpreted-text role="guilabel"} .
3.  Repeat as many times as needed.

![Define multiple actions to be executed](automated_actions/multi-actions.png)
...
role="guilabel"} to create a filter. - To only trigger the search for a
linked record after a minimum number of characters has been entered,
enter the desired number in the `Typeahead search`{.interpreted-text
role="guilabel"} field. In situations where the data set is large, this
can enhance both search relevancy and performance.
::::
...
To only trigger the search for the linked record after a minimum number
of characters has been entered, enter the desired number in the
`Typeahead search`{.interpreted-text role="guilabel"} field. In
situations where the data set is large, this can enhance both search
...
- `Date format`{.interpreted-text role="guilabel"}: By default the date
  will be shown as [Apr 2, 2025, 08:05 AM]{.title-ref}. Enable this
  property to show the date in the format [4/2/2025
  08:05:00]{.title-ref}. The numeric mode is the format set on the
  current language. In this mode the seconds are always shown.
- `Show date`{.interpreted-text role="guilabel"}: This property is
  enabled by default for `Date & Time`{.interpreted-text
  role="guilabel"} fields. Disable this property to show only the time.
...
- `Show seconds`{.interpreted-text role="guilabel"}: This property is
  disabled by default for `Date & Time`{.interpreted-text
  role="guilabel"} fields. Enable the property to show the seconds.
...
- To prevent users from creating, editing, deleting or duplicating
  records, untick `Can Create`{.interpreted-text role="guilabel"},
  `Can Edit`{.interpreted-text role="guilabel"},
  `Can Delete`{.interpreted-text role="guilabel"} or
  `Can Duplicate`{.interpreted-text role="guilabel"}.
...
- To change a button\'s label or style, click the button and edit its
  ...
  [btn-primary]{.title-ref} for a *primary* button or
  [btn-secondary]{.title-ref} for a *secondary* button) in the
  `Properties`{.interpreted-text role="guilabel"} tab.
  ...
  *Primary* buttons represent the main action(s) the user can take in a
  specific view, e.g., `Send`{.interpreted-text role="guilabel"} a
  request for quotation, and are more visually prominent. *Secondary*
  buttons offer alternative or less common actions, e.g.,
  `Print`{.interpreted-text role="guilabel"} or
  `Preview`{.interpreted-text role="guilabel"} a request for quotation,
  and are less visually prominent. By default, a new button is styled as
  a secondary button.
  ::::
...
::: example
![Quotation Template model\'s Form view](views/form-new-quotation.png)
:::
...
The widths of columns in a `List`{.interpreted-text role="guilabel"}
`oi-view-list`{.interpreted-text role="icon"} view are computed
automatically to provide the optimal user experience. However, it is
also possible to set a fixed column width per field. To do so, click on
the relevant column to open the field\'s `fa-server`{.interpreted-text
role="icon"} `Properties`{.interpreted-text role="guilabel"} tab, then
enter the desired number of pixels in the `Column Width
(px)`{.interpreted-text role="guilabel"} field.
