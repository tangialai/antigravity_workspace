# ODOO DOCUMENTATION SUMMARY - CATEGORY: ODOO18DOCS_APP_PRODUCTIVITY
...
`standard dashboards <dashboards/consult-dashboards/standard>`{.interpreted-text
...
Dashboards are customized by editing the dashboard\'s underlying
spreadsheet.
...
When customizing a
`standard dashboard <dashboards/consult-dashboards/standard>`{.interpreted-text
role="ref"}, it is highly recommended to `duplicate the dashboard
...
\- Users who do not have the appropriate `access rights
...
version of the dashboard\'s underlying spreadsheet. - To temporarily
unpublish a dashboard while you make changes, disable
...
finalized. - With `developer mode <developer-mode>`{.interpreted-text
role="ref"} activated, click on the `fa-pencil`{.interpreted-text
role="icon"} `(Edit)`{.interpreted-text role="guilabel"} icon beside the
name of a dashboard in the left panel to open its underlying
spreadsheet.
::::
...
Dashboard elements that are no longer needed can be deleted from the
spreadsheet. If, after deleting a dashboard element, a
...
Inserting new Odoo data into a dashboard\'s underlying spreadsheet
requires starting from the relevant Odoo view. To do so:
...
A list or pivot table is inserted into a new sheet in the dashboard\'s
underlying spreadsheet; a chart is inserted on the first sheet of the
spreadsheet.
...
glance without having to first navigate through multiple apps. Unlike
other Odoo dashboards, My Dashboard is not based on **Odoo
Spreadsheet**.
...
It is not possible to change the domain, i.e., the filtering or
grouping, of a view that has been added to My Dashboard. To change the
domain, make the necessary changes in the original view, then re-insert
the view in My Dashboard and delete the originally inserted view.
::::
...
**Odoo Dashboards** allows you to consult, customize, and build
interactive dashboards that display real-time data from your Odoo
...
- `consult dashboards <dashboards/consult-dashboards>`{.interpreted-text
  role="ref"}, including `standard,
  pre-configured dashboards <dashboards/consult-dashboards/standard>`{.interpreted-text
  role="ref"}
- `interact with dashboards <dashboards/use-dashboards/interact>`{.interpreted-text
  role="ref"} using filters and by accessing underlying data
...
## Consult dashboards {#dashboards/consult-dashboards}

On the main Dashboards page, the left panel lists all
`dashboards a user has access to
<dashboards/access-and-sharing>`{.interpreted-text role="ref"}, grouped
by section. Clicking on a dashboard name opens that dashboard in the
...
Clicking the `fa-angle-double-left`{.interpreted-text role="icon"}
`(double chevron)`{.interpreted-text role="guilabel"} icon at the top of
the left panel collapses the panel, maximizing the space available for
dashboards.
::::

### Standard dashboards {#dashboards/consult-dashboards/standard}
...
relevant insights for the topic in question. Data on specific aspects of
the topic is presented in tables and charts, while dashboard-specific
filters allow users to tailor the view to their needs.

::: example
Within the `Sales`{.interpreted-text role="guilabel"} section in the
Dashboards app, the `Sales`{.interpreted-text role="guilabel"} dashboard
gives an overview of the number of quotations and orders, the revenue,
and the average order value, as well as a chart showing monthly sales.
It also includes tables listing top quotations and sales orders,
top-performing products and salespeople, and top countries served.

A series of pre-configured global filters at the top of the dashboard
allows the entire dashboard to be filtered by, e.g., product or sales
team. A default value of [Last 90 days]{.title-ref} in the period filter
means data from the previous 90 days is automatically retrieved every
time the dashboard is opened or refreshed.

![Overview of Sales dashboard](dashboards/sales-dashboard.png)
:::
...
### Interact with dashboards {#dashboards/use-dashboards/interact}

In addition to consulting a dashboard for a high-level overview of key
business data, it is also possible to interact with the dashboard for a
more detailed analysis:

- **Filter data**: Most standard dashboards have one or more
  `global filters
  <spreadsheet/work_with_data/global_filters>`{.interpreted-text
  role="doc"}, shown as dropdown menus, at the top of the dashboard.
  These filters allow all the data on the dashboard to be filtered at
  the same time, for example, to show data only for a specific period of
  time, or for one or more salespeople or customers.
...
breadcrumb at the upper left of the page.
::::
...
- **Select a company** if, in a `multi-company
  ...
  one company. If this field is left blank, the dashboard is visible to
  all users with the appropriate access rights, regardless of which
  company is currently selected in the database.
...
- **Add a new dashboard to the section** by clicking
  ...
  - To start creating a dashboard from scratch, select
    ...
    the section overview, click the name of the section at the top left
    of the page. Update the `Group`{.interpreted-text role="guilabel"}
    and `Company`{.interpreted-text role="guilabel"} fields if needed.
  ...
  Edit this if needed via the configuration page of the relevant
  dashboard section.
...
### Consulting dashboards {#dashboards/access-and-sharing/viewing}
...
in the left-hand panel on the main Dashboards page. Users with
...
`access rights <access-rights/users>`{.interpreted-text role="ref"} to
the model from which the data has been retrieved, and takes into account
any record rules that may restrict access.
...
user must have both [Dashboards / Admin]{.title-ref} and [Documents /
User]{.title-ref} access rights.

### Share dashboard snapshot {#dashboards/access-and-sharing/sharing}
...
Chat** conversations, the **Discuss** app, and the *Chatter* composer.
This includes direct message conversations, channel conversations, and
**WhatsApp** messages.
...
In the `Description`{.interpreted-text role="guilabel"} field, add any
information that provides context for this response, such as guidelines
for when it should or should not be used.
...
Canned Responses`{.interpreted-text role="menuselection"}. Click into
the search bar, and remove any filters.
::::
...
includes direct message conversations, channel conversations, and
*WhatsApp* messages.

To use a canned response, type a colon ([:]{.title-ref}) into a
*Chatter* composer or chat window, followed by the shortcut. Then press
`Enter`{.interpreted-text role="kbd"}. This replaces the shortcut with
the substitution, though the response can still be edited before it is
sent.
...
Typing [:]{.title-ref} in the *Chatter* composer, or chat window, on its
own generates a drop-down list of available canned responses. A response
can be selected from the list, in addition to the use of shortcuts.

To search through the list of available responses, type [:]{.title-ref},
followed by the first few letters of the shortcut.
...
external contacts. Click the `fa-info-circle`{.interpreted-text
role="icon"} (`Info & Tags`{.interpreted-text role="guilabel"}) button
in the upper-right corner next to the view to access it.
::::
...
Enabling file centralization for a specific app automatically organizes
all associated files into dedicated folders. To do so, go to
`Documents --> Configuration --> Settings`{.interpreted-text
role="menuselection"}. For example, enabling
`Human Resources`{.interpreted-text role="guilabel"} makes HR documents
automatically available in the `HR`{.interpreted-text role="guilabel"}
folder, while documents related to Payroll are automatically available
in the `Payroll`{.interpreted-text role="guilabel"} sub-folder. Select
the desired folder from the dropdown list and select the
`Tags <documents/tags>`{.interpreted-text role="ref"} to be added to the
relevant files.
...
When centralizing accounting files, click `Journals`{.interpreted-text
role="guilabel"} to configure specific sub-folders for individual
journals.
::::
...
\- Changing the folder or tags does not affect existing files; the
changes will apply only to newly created ones. - If file centralization
is enabled for an app, deleting a record in that app moves its
...
- `fa-share-alt`{.interpreted-text role="icon"}
  ...
  `Share the folder or manage its access rights
  <documents/access-rights>`{.interpreted-text role="ref"}.

- `fa-external-link-square`{.interpreted-text role="icon"}
  `Add shortcut`{.interpreted-text role="guilabel"}: This option is only
  available for sub-folders and allows to create a shortcut to a
  sub-folder.

  - If you have editing permissions, the shortcut is created in the same
    folder.
  - If you don\'t have editing permissions, the shortcut appears in the
    `fa-hdd-o`{.interpreted-text role="icon"}
    `My Drive`{.interpreted-text role="guilabel"} section.
...
Setting up custom actions and automation rules may impact your [pricing
plan](https://www.odoo.com/pricing).
::::
...
- the `fa-cog`{.interpreted-text role="icon"} `Action`{.interpreted-text
  role="guilabel"} menu, which includes the options described below
- `Share`{.interpreted-text role="guilabel"}: to
  `share the file or manage its access rights <documents/access-rights>`{.interpreted-text
  role="ref"}
...
role="icon"} `Action`{.interpreted-text role="guilabel"} menu:
...
To divide a PDF into individual or groups of pages, open the PDF and
click `fa-scissors`{.interpreted-text role="icon"}
`Split PDF`{.interpreted-text role="guilabel"} in the upper-right part
of the document preview. Click the `fa-scissors`{.interpreted-text
role="icon"} (`scissors`{.interpreted-text role="guilabel"}) icon
between pages to remove a split if needed, then click
`Split`{.interpreted-text role="guilabel"} to confirm.

![Split a PDF](documents/split-pdf.png)

To merge PDF files, follow these steps:

1.  Navigate to the folder containing the files you want to merge, then
    switch to the list view and select the relevant files.
2.  Click the `fa-cog`{.interpreted-text role="icon"}
    `Action`{.interpreted-text role="guilabel"} button and select
    `fa-scissors`{.interpreted-text role="icon"}
    `Merge PDFs`{.interpreted-text role="guilabel"}.
3.  If needed, click `Add file`{.interpreted-text role="guilabel"} to
    browse and select a PDF file from your computer.
4.  Click the `fa-scissors`{.interpreted-text role="icon"}
    (`scissors`{.interpreted-text role="guilabel"}) icon between the
    files.
5.  Click `Split`{.interpreted-text role="guilabel"} to merge them.
...
3.  If needed, set a `Due Date In`{.interpreted-text role="guilabel"},
    choose the `Folder`{.interpreted-text role="guilabel"} where the
    file should be added, add `Tags`{.interpreted-text role="guilabel"},
    and write a `Message`{.interpreted-text role="guilabel"}.
...
or file, then click the `fa-info-circle`{.interpreted-text role="icon"}
...
- Change the file\'s or folder\'s `Owner`{.interpreted-text
  role="guilabel"} and `Contact`{.interpreted-text role="guilabel"}. By
  default, the person who creates a file or folder is set as its
  `Owner`{.interpreted-text role="guilabel"} and granted full access
  rights to it. To change it, select the required user from the dropdown
  list. The `Contact`{.interpreted-text role="guilabel"} is a person who
  only has `Viewer`{.interpreted-text role="guilabel"} `access rights
  <documents/access-rights>`{.interpreted-text role="ref"} to the file
  or folder, e.g., an existing supplier in the database.
  ...
  `Contact`{.interpreted-text role="guilabel"} and have at least
...
2.  Click the `fa-info-circle`{.interpreted-text role="icon"}
    ...
    upper-right corner next to the view icons.
3.  In the details panel, enter the desired `Email`{.interpreted-text
    role="guilabel"} alias and select or create the domain.
4.  Optionally, specify an `Activity type`{.interpreted-text
    role="guilabel"} and assignee to create an `activity
    </applications/essentials/activities>`{.interpreted-text role="doc"}
    when a file is received via the alias.
5.  Optionally, select the `Tags <documents/tags>`{.interpreted-text
...
`fa-info-circle`{.interpreted-text role="icon"}
(`Info & Tags`{.interpreted-text role="guilabel"}) in the upper-right
corner next to the view icons, and then, in the details panel, select a
tag from the dropdown list.
...
## Share and access rights {#documents/access-rights}
...
In the `Share`{.interpreted-text role="guilabel"} pop-up, grant access
to specific users or contacts by selecting their name from the dropdown
menu or by adding their email address manually, then select
...
To set `General access`{.interpreted-text role="guilabel"} for
`Internal users`{.interpreted-text role="guilabel"} or `Anyone with the
...
restrict access completely). For
`Anyone with the link`{.interpreted-text role="guilabel"}, you can
further specify whether the folder or file should be
`Discoverable`{.interpreted-text role="guilabel"} (accessible through
browsing) or require that users
`Must have the link to access`{.interpreted-text role="guilabel"} it.
...
\- Public users `Must have the link to access`{.interpreted-text
role="guilabel"} a folder or file on the portal when connecting for the
first time. - Each folder and file URL includes the access rights that
have been set for it. When you share a folder, the person you share it
with is directed to a dedicated portal where they can view the files in
that folder, excluding any with restricted access.
::::
...
`Portal users </applications/general/users/portal>`{.interpreted-text
...
To create an article from scratch, click `New`{.interpreted-text
role="guilabel"} in the top right corner or hover over the
`Private`{.interpreted-text role="guilabel"} or
`Workspace`{.interpreted-text role="guilabel"} category in the sidebar
tree, then click the `fa-plus`{.interpreted-text role="icon"}
`(plus)`{.interpreted-text role="guilabel"} icon. Start typing text or
...
Click `fa-commenting`{.interpreted-text role="icon"}
...
role="icon"} `(ellipsis)`{.interpreted-text role="guilabel"} icon, then
`Add Cover`{.interpreted-text role="guilabel"}. The following options
enable selecting and inserting pictures from different sources:

- Search the
  `Unsplash </applications/general/integrations/unsplash>`{.interpreted-text
  role="doc"} database to find a suitable picture. If the database and
  **Unsplash** account are associated, the cover picture is
  automatically selected based on the article\'s name.
- `Add URL`{.interpreted-text role="guilabel"}: Copy-paste the **image
  address**.
- `Upload an image`{.interpreted-text role="guilabel"}: Upload the file
  into the image library.

To manage the cover picture, hover the mouse over it and select the
preferred option:

- `Replace Cover`{.interpreted-text role="guilabel"} and search from the
  database or library, or add a different URL.
- `Reposition`{.interpreted-text role="guilabel"} and adjust the picture
  before clicking `Save Position`{.interpreted-text role="guilabel"}.
...
- Click the `fa-ellipsis-v`{.interpreted-text role="icon"}
  `(ellipsis)`{.interpreted-text role="guilabel"} icon, then
...
- Clicking the inserted link opens a pop-up with the view\'s name next
  ...
  (`copy`{.interpreted-text role="guilabel"}),
  `fa-pencil-square-o`{.interpreted-text role="icon"}
  (`edit`{.interpreted-text role="guilabel"}), and
  `fa-chain-broken`{.interpreted-text role="icon"}
  (`remove`{.interpreted-text role="guilabel"}) icons. Click the name
  inside the pop-up to open the linked view.
::::
...
Click the `fa-ellipsis-v`{.interpreted-text role="icon"}
(`ellipsis`{.interpreted-text role="guilabel"}) icon and select one of
the following actions for basic article management:
...
- `Export`{.interpreted-text role="guilabel"}: Open the browser\'s print
  function.
...
dropping it under another article or category. - Press
...
::: tabs
  ...
  `No access`{.interpreted-text   Prevent all users from accessing the article in the
  role="guilabel"}                sidebar tree or searching in the command palette.
  ----------------------------------------------------------------------------------------
:::

##### Visibility {#knowledge/articles_editing/visibility}
...
::: tabs
  ...
  `Members`{.interpreted-text    The article is only visible in the sidebar tree to
  role="guilabel"}               `invited users
...
To grant specific internal or portal users access to a private article
or to share a `Workspace`{.interpreted-text role="guilabel"} article
with a portal user, follow these steps:
...
Click `Share`{.interpreted-text role="guilabel"} and activate the
`Share to web`{.interpreted-text role="guilabel"} toggle to generate a
URL. Click the `fa-clone`{.interpreted-text role="icon"}
(`copy`{.interpreted-text role="guilabel"}) icon to copy the article\'s
URL.
...
`fa-ellipsis-v`{.interpreted-text role="icon"}
(`ellipsis`{.interpreted-text role="guilabel"}) icon, then
`Send to Trash`{.interpreted-text role="guilabel"}. The article is moved
to the trash for 30 days before being permanently deleted.

To permanently delete an article, click `Search`{.interpreted-text
...
click `Search`{.interpreted-text role="guilabel"} in the top-left menu.
In the search bar, click `Filters --> Trashed`{.interpreted-text
...
Click `Search`{.interpreted-text role="guilabel"}, select an article,
...
To restore an archived article, click `Search`{.interpreted-text
...
Retrieving Knowledge articles consists of accessing them from various
Odoo apps or restoring previous versions.
...
tree and click the `fa-history`{.interpreted-text role="icon"}
(`history`{.interpreted-text role="guilabel"}) icon in the top-right
menu to open the version history. Select a version and click
`Restore history`{.interpreted-text role="guilabel"}.
...
users with [Can edit]{.title-ref}
`access rights <knowledge/articles_editing/rights>`{.interpreted-text
role="ref"} can add to `nested articles
<knowledge/articles_editing/create-article>`{.interpreted-text
...
role="icon"} (`ellipsis`{.interpreted-text role="guilabel"}) icon, then
`Add Properties --> Add a Property`{.interpreted-text
...
</applications/essentials/property_fields>`{.interpreted-text
role="doc"}.
...
role="guilabel"} section, fill in the `Valid Until`{.interpreted-text
role="guilabel"} field, toggle the `Reminder`{.interpreted-text
role="guilabel"} switch, and click the value to edit the default number
of days between reminders.

![Set the number of days between reminders](sign/reminder.png)
...
2.  Click `New`{.interpreted-text role="guilabel"} and select
    ...
    which `Workspace`{.interpreted-text role="guilabel"} (i.e., folder)
    the spreadsheet should be created. If
    `file centralization <documents/file-centralization>`{.interpreted-text
    role="ref"} has been enabled for spreadsheets, you will also have
    the option to save the new spreadsheet in the
    `Spreadsheet`{.interpreted-text role="guilabel"} folder.
    ::::
...
`fa-hdd-o`{.interpreted-text role="icon"} `My Drive`{.interpreted-text
role="guilabel"} personal folder or, if `file centralization
<documents/file-centralization>`{.interpreted-text role="ref"} has been
enabled for spreadsheets, in the `Spreadsheet`{.interpreted-text
role="guilabel"} folder.
::::
...
Spreadsheets in [.xlsx]{.title-ref} format can be uploaded into Odoo
Documents and opened as an Odoo spreadsheet. To do so:

1.  Open Odoo Documents and navigate to the section or folder where the
    spreadsheet should be saved.
2.  Click `New`{.interpreted-text role="guilabel"} and select
    `Upload`{.interpreted-text role="guilabel"}.
3.  Select the relevant [.xlsx]{.title-ref} file and click
    `Open`{.interpreted-text role="guilabel"}.
4.  Click on the uploaded file.
5.  By default, the original [.xlsx]{.title-ref} file is deleted when it
    is opened as an Odoo Spreadsheet. To preserve the original file in
    the same folder in Odoo Spreadsheet, disable `Send source
    file to trash`{.interpreted-text role="guilabel"}.
6.  Click `Open with Odoo Spreadsheet`{.interpreted-text
    role="guilabel"}.

The file is now an Odoo spreadsheet and can be fully edited in Odoo
Spreadsheet.
...
- `os-download`{.interpreted-text role="icon"}
...
- `os-add-to-dashboard`{.interpreted-text role="icon"}
  ...
  <spreadsheet/get-started/convert-to-dashboard>`{.interpreted-text
  role="ref"} the current spreadsheet into an Odoo dashboard.
...
Clicking on a data source opens the related properties in a pane on the
...
\- The properties pane can also be opened by right-clicking any cell of
...
when navigating between spreadsheet tabs. To close the properties pane,
...
4.  Click `Blank spreadsheet`{.interpreted-text role="guilabel"} or
    select in which existing spreadsheet the list should be inserted.
    ...
    New spreadsheets are saved in the **Odoo Documents** app in either
    the `fa-hdd-o`{.interpreted-text role="icon"}
    `My Drive`{.interpreted-text role="guilabel"} personal workspace or,
    if `file centralization
    <documents/file-centralization>`{.interpreted-text role="ref"} has
    been enabled for spreadsheets, in the
    `Spreadsheet`{.interpreted-text role="guilabel"} workspace.
    ::::
...
e.g., *Quotations by Total (List #1)*. A pane on the right side of the
screen shows the
...
\- To view an individual record of an inserted list, right-click on any
cell of the relevant row, then click `fa-eye`{.interpreted-text
role="icon"} `See record`{.interpreted-text role="guilabel"}. To return
to the spreadsheet, click the name of the spreadsheet in the breadcrumbs
at the top of the view. - To sever the link between an inserted list and
your database, select the entire list, right-click and select
`fa-clone`{.interpreted-text role="icon"} `Copy`{.interpreted-text
...
Once a list from an Odoo database has been inserted into an Odoo
spreadsheet, you can:
...
- From the
  `properties pane <spreadsheet/insert/list-properties>`{.interpreted-text
...
3.  Click `Blank spreadsheet`{.interpreted-text role="guilabel"} or
    select in which existing spreadsheet the pivot table should be
    inserted.
...
A pane on the right side of the screen shows the `pivot table properties
<spreadsheet/insert/pivot-table-properties>`{.interpreted-text
role="ref"}.
...
\- To view the records referenced by an individual cell of a pivot
table, right-click on the cell, then click `fa-eye`{.interpreted-text
role="icon"} `See record`{.interpreted-text role="guilabel"}. To return
to the spreadsheet, click the name of the spreadsheet in the breadcrumbs
at the top of the view. - To sever the link between an inserted pivot
table and your database, select the entire pivot table, right-click and
select `fa-clone`{.interpreted-text role="icon"}
`Copy`{.interpreted-text role="guilabel"}, then right-click again and
select `Paste special --> Paste as value`{.interpreted-text
...
role="icon"} `(gear)`{.interpreted-text role="guilabel"} icon then
`fa-copy`{.interpreted-text role="icon"} `Duplicate`{.interpreted-text
role="guilabel"} or `fa-trash`{.interpreted-text role="icon"}
`Delete`{.interpreted-text role="guilabel"}.
...
2.  Edit the `Name`{.interpreted-text role="guilabel"} in the properties
    pane and the sheet tab if needed.
...
- Delete the spreadsheet table using your preferred means, e.g., via
  ...
  deletes the visual representation of the data.
- From the
  `properties pane <spreadsheet/insert/pivot-table-properties>`{.interpreted-text
...
3.  Click `Blank spreadsheet`{.interpreted-text role="guilabel"} or
    select in which existing spreadsheet the chart should be inserted.
...
`chart properties <spreadsheet/visualize/chart-properties>`{.interpreted-text
...
2.  At the bottom of the `fa-sliders`{.interpreted-text role="icon"}
    ...
    properties pane, click under `Link to Odoo menu`{.interpreted-text
...
- Any user with access to a spreadsheet can download it as an
  ...
  role="menuselection"} `os-download`{.interpreted-text role="icon"}
...
data, click `fa-share-alt`{.interpreted-text role="icon"}
`Freeze and share`{.interpreted-text role="guilabel"} in the upper-right
corner above the spreadsheet, then
`configure access as appropriate <documents/access-rights>`{.interpreted-text
role="ref"}.
...
2.  Type a comment in the text box.
    ...
    Type [@]{.title-ref} to tag users, or add emojis or GIFs by clicking
    `oi-smile-add`{.interpreted-text role="icon"} `Add a
    Reaction`{.interpreted-text role="guilabel"}.
    ::::

3.  Click `Send`{.interpreted-text role="guilabel"} or press
    [Ctrl]{.title-ref} + [Enter]{.title-ref}.
...
2.  Click `Insert --> Chart`{.interpreted-text role="menuselection"}
    ...
    properties <spreadsheet/visualize/chart-properties>`{.interpreted-text
...
:::::::: tabs
::: {.tab alt="Line chart icon"}
Line .. image:: charts/chart-type-line.png
...
## Chart properties {#spreadsheet/visualize/chart-properties}

When you
`create a chart manually <spreadsheet/visualize/create-chart>`{.interpreted-text
role="ref"} or `insert a
chart from an Odoo graph view <spreadsheet/insert/chart>`{.interpreted-text
role="ref"} into a spreadsheet, the chart properties appear in a pane on
the right side of the spreadsheet. Here, it is possible to modify
various aspects of the chart\'s configuration and design via the
`fa-sliders`{.interpreted-text role="icon"} `Configuration
<spreadsheet/visualize/charts-configuration>`{.interpreted-text
role="ref"} and `fa-paint-brush`{.interpreted-text role="icon"} `Design
<spreadsheet/visualize/charts-design>`{.interpreted-text role="ref"}
tabs respectively.
...
The `General`{.interpreted-text role="guilabel"} section lets you modify
...
- `Chart title`{.interpreted-text role="guilabel"}: Edit the chart
  title, if needed. The font formatting, horizontal alignment and color
  of the title can be modified using the editor.
- `Vertical axis position`{.interpreted-text role="guilabel"}: Choose
  whether the vertical axis is placed on the left or right in line,
...
- Enable `Show trend line`{.interpreted-text role="guilabel"} to add a
  trend line to line, column, and area charts.

For line, column, and area charts, the `Axis`{.interpreted-text
role="guilabel"} section lets you add a title to one or both axes. The
font formatting, horizontal alignment, and color of the title can be
...
## Filter {#spreadsheet/functions/filter}
  ...
  SORT(range, \[sort_column, \...\], \[is_ascending, \...\])   [Excel SORT article](https://support.microsoft.com/en-us/office/sort-function-22f63bd0-ccc8-492f-953d-c20e8e44b86c)
...
## Lookup {#spreadsheet/functions/lookup}
  ...
  OFFSET(reference, rows, cols, \[height\], \[width\])                                                 [Excel OFFSET article](https://support.microsoft.com/en-us/office/offset-function-c8de19ae-dd79-4b9b-a14e-b4d906d11b66)
  PIVOT(pivot_id, \[row_count\], \[include_total\], \[include_column_titles\], \[column_count\])       Get a pivot table (not compatible with Excel)
  PIVOT.HEADER(pivot_id, \[domain_field_name, \...\], \[domain_value, \...\])                          Get the header of a pivot table (not compatible with Excel)
  PIVOT.VALUE(pivot_id, measure_name, \[domain_field_name, \...\], \[domain_value, \...\])             Get the value from a pivot table (not compatible with Excel)
...
## Math {#spreadsheet/functions/math}
  ...
  LOG(value, \[base\])                                                                              Get the logarithm of a number for a given base (not compatible with Excel)
  ...
  SEQUENCE(rows, \[columns\], \[start\], \]\[step\])                                                [Excel SEQUENCE article](https://support.microsoft.com/en-us/office/sequence-function-57467a98-57e0-4817-9f14-2eb78519ca90)
...
## Parser {#spreadsheet/functions/parser}
  ...
  Name and arguments                    Description or link
  ------------------------------------- ---------------------------------------------------------------------------------------------------------------------------
  CONVERT(number, from_unit, to_unit)   [Excel CONVERT article](https://support.microsoft.com/en-us/office/convert-function-d785bef1-808e-4aac-bdcd-666c810f9af2)
...
## Statistical {#spreadsheet/functions/statistical}
  ...
  MATTHEWS(data_x, data_y)                                                                                  Compute the Matthews correlation coefficient of a dataset (not compatible with Excel)
...
## Text {#spreadsheet/functions/text}
  ...
  VALUE(text)                                                                   [Excel VALUE article](https://support.microsoft.com/en-us/office/value-function-257d0108-07dc-437d-ae1c-bc2d3953d8c2)
...
### Financial
  ...
  ODOO.FISCALYEAR.START(day, \[company_id\])                                                                               Returns the starting date of the fiscal year encompassing the provided date (not compatible with Excel)
  ODOO.FISCALYEAR.END(day, \[company_id\])                                                                                 Returns the ending date of the fiscal year encompassing the provided date (not compatible with Excel)
  ODOO.PARTNER.BALANCE(partner_ids, \[account_codes\], \[date_range\], \[offset\], \[company_id\], \[include_unposted\])   Get the partner balance for the specified account(s) and period (not compatible with Excel)
  ODOO.RESIDUAL(\[account_codes\], \[date_range\], \[offset\], \[company_id\], \[include_unposted\])                       Get the residual amount for the specified account(s) and period (not compatible with Excel)
...
When a spreadsheet with global filters is added to a dashboard, the
filters appear as dropdown menus at the top of the dashboard. In a
spreadsheet, they appear in a pane to the right of the spreadsheet.
...
To access the underlying spreadsheet of a dashboard, with the
**Dashboards** app open,
`activate developer mode <developer-mode>`{.interpreted-text
role="ref"}, then click the `fa-pencil`{.interpreted-text role="icon"}
`(Edit)`{.interpreted-text role="guilabel"} icon that appears when
hovering over the dashboard name.
::::

To add a new filter, click `os-global-filters`{.interpreted-text
role="icon"} `Filters`{.interpreted-text role="guilabel"}, then, under
`Add
a new filter...`{.interpreted-text role="guilabel"} click
`Date`{.interpreted-text role="guilabel"}, `Relation`{.interpreted-text
role="guilabel"}, or `Text`{.interpreted-text role="guilabel"} as
appropriate. The `Filter properties`{.interpreted-text role="guilabel"}
pane opens.
...
2.  From the `Time range`{.interpreted-text role="guilabel"} dropdown
    menu, select one of the following:

    - `Month / Quarter`{.interpreted-text role="guilabel"}: enables a
      dropdown menu of specific months and/or quarters and a year
      selector for the year. The values `Months`{.interpreted-text
      role="guilabel"} and `Quarters`{.interpreted-text role="guilabel"}
      are enabled by default. Disabling both of these values allows
      filtering by year only.

      To set a `Default value`{.interpreted-text role="guilabel"},
      enable
      `Automatically filter on the current period`{.interpreted-text
      role="guilabel"} and choose whether to filter on the current
      `Month`{.interpreted-text role="guilabel"},
      `Quarter`{.interpreted-text role="guilabel"} or
      `Year`{.interpreted-text role="guilabel"}.

    - `Relative Period`{.interpreted-text role="guilabel"}: enables a
      dropdown menu of specific time ranges relative to the current date
      (e.g., `Year to Date`{.interpreted-text role="guilabel"},
      `Last 7 Days`{.interpreted-text role="guilabel"},
      `Last 30 Days`{.interpreted-text role="guilabel"}, etc.).

      To set a `Default value`{.interpreted-text role="guilabel"},
      select one of the available values.

    - `From / To`{.interpreted-text role="guilabel"}: enables
      `Date from...`{.interpreted-text role="guilabel"} and
      `Date to...`{.interpreted-text role="guilabel"} date selection
      fields to define a specific time range (e.g.,
      [06/05/2024]{.title-ref} to [06/27/2024]{.title-ref}).

3.  In the `Field matching`{.interpreted-text role="guilabel"} section,
    for each data source, click below `Date
    field`{.interpreted-text role="guilabel"} and select the field the
    filter should match with.

    The `Period offset`{.interpreted-text role="guilabel"} feature
    enables comparisons to be made by shifting the time range by one or
    two periods in the past or future. By default, no period offset is
    set. The offsets available are: `Previous`{.interpreted-text
    role="guilabel"}, `Before previous`{.interpreted-text
    role="guilabel"}, `Next`{.interpreted-text role="guilabel"}, and
...
filter has been created to allow the pivot table and chart to show sales
data per quarter. If only a year is selected, data is shown for the
entire year.

![A date filter to filter on quarter and year](global_filters/example-date.png)
...
matching date field. A matching date field is not needed for *List #1*
as we will not use this filter on the data source in question.
...
filter has been created to allow the pivot table and chart to show sales
data related to selected salespeople only. The *User* model was set as
the `Related model`{.interpreted-text role="guilabel"}.
...
assigned as the matching field for both the pivot table and the chart. A
matching field is not needed for *List #1* as we will not use this
filter on the data source in question.
...
role="ref"} or
`Decimal (float) <studio/fields/simple-fields-decimal>`{.interpreted-text
role="ref"} field.
::::
...
2.  Optionally, enable `Restrict values to a range`{.interpreted-text
    ...
    either by typing the range or selecting it from within the
    spreadsheet.
...
filter was created to allow the user to select a product from the
`Product`{.interpreted-text role="guilabel"} filter and have both the
pivot table and chart only show sales data related to that specific
product.

![Global filters set on a pivot table](global_filters/example-text.png)

In the `Filter properties`{.interpreted-text role="guilabel"}, the
`Possible values`{.interpreted-text role="guilabel"} of the filter were
restricted to the range `'Products (List #1)'!A2:A34`{.interpreted-text
role="guilabel"}. This corresponds to the range containing the
`Display name`{.interpreted-text role="guilabel"} of the product on a
list inserted in the spreadsheet.

![A text filter with a restricted range](global_filters/field-matching-text.png)

With this configuration, the pivot table and chart can be filtered by
product name by selecting one of the predefined values available in the
text filter. In this case, `Furniture`{.interpreted-text
role="guilabel"} has already been selected as the
`Product category`{.interpreted-text role="guilabel"}, meaning that only
products of this category can be selected as possible values.

Furthermore, if the values in the range have been retrieved dynamically
from the database, as in this case, the text filter is also dynamic,
i.e., will reflect changes made to those values.
:::
...
<../../applications/productivity/dashboards>`{.interpreted-text
role="doc"}. On a dashboard, charts and data tables are used to display
dynamic Odoo data and provide an overview of key business metrics.

`Standard, pre-configured dashboards
<dashboards/consult-dashboards/standard>`{.interpreted-text role="ref"}
can be `customized
<build_and_customize_dashboards/customize>`{.interpreted-text
